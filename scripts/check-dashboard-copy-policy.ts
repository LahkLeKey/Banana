...
const DEFAULT_FILE_CANDIDATES = [
    [
        ...
        "src/typescript/react/src/pages/GameEnginePage.tsx",
    ],
    [
...
type Args = {
    files: string[]; termsPath : string; strict : boolean;
};

function resolveDefaultFiles(): string[]
{
    const resolved: string[] = [];
    for (const candidates of DEFAULT_FILE_CANDIDATES)
    {
        const existing = candidates.map((relativePath) => resolve(CWD, relativePath))
    ...
    {
        console.error("[check-dashboard-copy-policy] no default source files found");
        process.exit(2);
    }
...
type Violation = {
    file: string; line : number; term : string; snippet : string;
};
...
function main(): number
{
    ...
    {
        console.log(
            `[check-dashboard-copy-policy] OK — no restricted terms found in ${
                args.files.length} file(s)`,
        );
        return 0;
    }

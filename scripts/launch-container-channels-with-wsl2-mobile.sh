#!/usr/bin/env bash
# Spec 011 — Windows-shell mobile emulator launcher.
# Same distro-selection contract as the Electron launcher. Exit 42 = preflight.
set -euo pipefail
...
exec wsl.exe -d "$DISTRO" -- bash -lc \
  "cd /mnt/c/Github/Banana && bash scripts/compose-mobile-emulators-wsl2.sh"

...
TRIAGE_ISSUE_LABELS="${BANANA_TRIAGE_ISSUE_LABELS:-triage-idea,copilot-suggestion,ai-generated,automation,speckit-driven}"
TRIAGE_PR_LABELS="${BANANA_TRIAGE_PR_LABELS:-automation,triaged-item,copilot-auto-approve,copilot-autonomous-cycle,speckit-driven}"
...
ensure_issue_label_contract() {
  ...
  ensure_label "speckit-driven" "0B63A5" "Bypass human gate after Copilot triage while tracking spec-kit-driven integrations"
...
apply_issue_labels() {
  ...
  for label in "${labels_ref[@]}"; do
    ...
    if [[ "$label" == "triage-idea" || "$label" == "copilot-suggestion" || "$label" == "speckit-driven" || "$label" == "$HUMAN_TRIAGE_LABEL" || "$label" == "$SOURCE_HUMAN_ISSUE_LABEL" || "$label" == "$AI_GENERATED_LABEL" || "$label" == "$SOURCE_AI_ISSUE_LABEL" || "$label" == "$EPIC_LABEL" || "$label" == "$EPIC_DECOMPOSED_LABEL" || "$label" == "$USER_STORY_LABEL" || "$label" == "$TASK_LABEL" ]]; then
      ensure_issue_label_contract
      gh issue edit "$issue_number" --add-label "$label" >/dev/null 2>&1 || true
      continue
    fi
...
EOF
  ...
  if [[ "$TRIAGE_EPIC_AUTO_DISPATCH_FIRST_TASK" == "true" ]]; then
    gh issue edit "$task1_number" --add-label "triage-idea" --add-label "speckit-driven" >/dev/null 2>&1 || true
    dispatch_labels="triage-idea,automation,${TASK_LABEL},speckit-driven,${source_bucket_label},${source_label},agent:api-pipeline-agent"

...
if [[ -n "${BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_ID:-}" || -n "${BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_SECRET:-}" ]]; then
  if [[ -z "${BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_ID:-}" || -z "${BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_SECRET:-}" ]]; then
    echo "[keycloak-fly] ERROR: set both BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_ID and BANANA_KEYCLOAK_IDP_GITHUB_CLIENT_SECRET, or neither." >&2
    exit 1
  fi
fi
...
secret_args=(
  ...
  "KC_DB_PASSWORD=${BANANA_KEYCLOAK_DB_PASSWORD}"
)

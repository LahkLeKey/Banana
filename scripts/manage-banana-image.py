#!/usr/bin/env python3
"""Manage immutable banana model images and channel pointers."""
...
def promote_image(args: argparse.Namespace) -> int:
    ...
    if signal_score < float(args.min_signal_score):
        raise ValueError(
            f"Promotion blocked: signal_score={signal_score:.4f} below min_signal_score={args.min_signal_score}."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Manage banana model image registry.")
    ...
    promote.add_argument("--min-signal-score", type=float, default=0.7)

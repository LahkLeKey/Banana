...
- [ ] T001 Document the three tiers in `research.md`: local (`.env.local`), preview (per-PR Vercel URL), production (`banana.engineer`), with each tier's `VITE_BANANA_API_BASE_URL` value.
...
- [ ] T005 Confirm `BANANA_PG_CONNECTION` is set per Vercel environment tier (only applicable if the API is also on Vercel; otherwise document the API-side isolation strategy).
...
- [ ] T007 Document who owns production credentials and the rotation cadence in `research.md`.
- [ ] T008 Confirm production secrets are NOT exposed in preview build logs or preview environment variables.

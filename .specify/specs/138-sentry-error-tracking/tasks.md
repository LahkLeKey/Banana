...
- [x] T004 Set `VITE_SENTRY_DSN` and `SENTRY_AUTH_TOKEN` as Vercel Production environment variables.
- [x] T005 Run `bun run build --cwd src/typescript/react` and confirm source maps are uploaded to Sentry (check Sentry → Releases for the new release).
- [x] T006 Trigger a test error in the deployed preview and confirm a readable stack trace appears in Sentry.

...
- [x] T001 Repoint `.specify/feature.json` -> `035-banana-corpus-and-trainer`.
- [x] T002 Create branch `feature/035-banana-corpus-and-trainer`
  from current head.

## Phase 2 -- Corpus

- [x] T003 Author `data/banana/corpus.json` with >=12 banana +
  >=6 not-banana samples conforming to the SPIKE 034 schema.
- [x] T004 Add a corpus-schema lint check that fails when
  `data/<model>/corpus.json` violates the SPIKE 034 contract
  (or document deferral in `analysis/schema-lint-deferral.md`).

## Phase 3 -- Trainer

- [x] T005 Add `scripts/train-banana-model.py` mirroring the
  not-banana CLI surface (FR-035-03).
- [x] T006 Ensure determinism: seed RNG, sort vocabulary
  emission, normalize timestamp / metadata fields. Add
  `analysis/determinism-notes.md` listing every non-deterministic
  source eliminated.
- [x] T007 Run the trainer twice with the same flags; assert
  byte-identical output (FR-035-04).

## Phase 4 -- Image management

- [x] T008 Add `scripts/manage-banana-image.py` mirroring
  `manage-not-banana-model-image.py` (promote, list,
  current-channel pointer).
- [x] T009 Promote the first banana model image to
  `data/banana/images/<sha>.tar` + channel pointer JSON; commit.

## Phase 5 -- Docs + close-out

- [x] T010 Author `README.md` execution tracker (run command, how

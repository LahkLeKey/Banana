...
- [ ] T001 Add a `no-cache` header entry in `vercel.json` for `source: "/index.html"` alongside the existing asset `immutable` entry.
- [ ] T002 Validate `vercel.json`: `python3 -c "import json; json.load(open('vercel.json')); print('OK')"`.
...
- [ ] T006 Run `python scripts/validate-spec-tasks-parity.py .specify/specs/135-cache-headers-cdn` and confirm `OK`.

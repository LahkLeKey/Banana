...
- [x] T001 Repoint `.specify/feature.json` -> `032-frontend-f5-launch-profiles-spike`.
- [x] T002 Confirm clean tree on `feature/032-launch-profiles-and-model-suite-spikes`.

## Phase 2 -- Investigation

- [x] T003 Author `analysis/launch-surface-matrix.md` -- per-channel
  cwd, command, env vars, prerequisites.
- [x] T004 Author `analysis/f5-contract-decision.md` -- compound vs
  single, env-var seam, container-contract boundary.

## Phase 3 -- Initial F5 contract

- [x] T005 Add `.vscode/launch.json` with four single-channel
...
- [x] T006 Manual smoke -- F5 `API + React`, hit Vite URL, confirm
  classifier round-trip works.

## Phase 4 -- Close-out

- [x] T007 Author `analysis/followup-readiness.md` for slice 035
  (debugger attach + test runner launches).
- [x] T008 Run `bash scripts/validate-spec-tasks-parity.sh --all`.
- [x] T009 Mark all tasks `[x]` and update README to COMPLETE.

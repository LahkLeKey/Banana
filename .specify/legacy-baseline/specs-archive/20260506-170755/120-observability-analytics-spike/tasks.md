...
- [x] T003 Deploy to a Vercel preview and open the Analytics tab in the Vercel dashboard to confirm page-view events are received.

## Phase 2: Sentry Browser SDK

- [x] T004 Create a Sentry free-tier project (type: React/Vite) and record the DSN.
...
- [x] T007 Throw a test error in development, confirm it appears in the Sentry dashboard with a readable stack trace.

## Phase 3: Uptime Monitoring

- [x] T008 Create a free UptimeRobot (or Better Uptime) monitor for `https://banana.engineer` with a 5-minute check interval and an email alert.
- [x] T009 Create a second monitor for `https://api.banana.engineer/health` (or the equivalent production health endpoint).

...
describe('training economy store fail-fast runtime contract', () => {
  test('throws when default postgres mode has no configured database url', async () => {
    ...
    try {
      ...
      await expect(getTrainingEconomyStore()).rejects.toThrow(
  ...
  test('supports explicit in-memory mode for controlled environments', async () => {
    const envSnapshot = snapshotEnv([
      'BANANA_TRAINING_STORE_MODE',
      'BANANA_PG_CONNECTION',
      'DATABASE_URL',
  ...
  test('rejects unknown training store mode', async () => {
    ...
    try {
      ...
      await expect(getTrainingEconomyStore()).rejects.toThrow(
          'Unsupported BANANA_TRAINING_STORE_MODE');
    } finally {
      restoreEnv(envSnapshot);
    }
  });

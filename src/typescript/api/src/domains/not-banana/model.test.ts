...
function createVocabularyArtifact(
  ...
  writeFileSync(
    filePath,
    JSON.stringify({
      schema_version: options?.schemaVersion ?? TRAINING_ARTIFACT_SCHEMA_VERSION,
  ...
  if (
    ...
    writeFileSync(
      metricsPath,
      JSON.stringify({
        schema_version: options?.metricsSchemaVersion ?? TRAINING_ARTIFACT_SCHEMA_VERSION,
        generated_at_utc: "2026-04-25T00:00:00Z",
        metrics: {
...
describe("not-banana trained model loader", () => {
  ...
  test("falls back to builtin tokens when artifact is missing", () => {
    __resetTrainedModelCacheForTests();
    process.env.BANANA_NOT_BANANA_MODEL_PATH = "artifacts/does-not-exist/vocabulary.json";
    ...
    expect(model.fallback_reason?.startsWith("artifact_parse_error:")).toBe(true);
  });
  ...
  test("falls back when vocabulary schema version is unsupported", () => {
    ...
    try {
      ...
      const model = loadTrainedModel(true);
      expect(model.source).toBe("builtin-fallback");
      expect(model.fallback_reason).toContain("unsupported_vocabulary_schema_version");
    } finally {
      rmSync(dirname(artifact), { recursive: true, force: true });
    }
  });
  ...
  test("falls back when metrics schema version is unsupported", () => {
    ...
    try {
      ...
      const model = loadTrainedModel(true);
      expect(model.source).toBe("builtin-fallback");
      expect(model.fallback_reason).toContain("unsupported_metrics_schema_version");
    } finally {
      rmSync(dirname(artifact), { recursive: true, force: true });
    }
  });
  ...
  test("returns neutral score for empty token input", () => {
    __resetTrainedModelCacheForTests();
    process.env.BANANA_NOT_BANANA_MODEL_PATH = join(
      tmpdir(),
      "banana-model-test-missing",
      "vocabulary.json"
    );
    const model = loadTrainedModel(true);
    expect(model.source).toBe("builtin-fallback");

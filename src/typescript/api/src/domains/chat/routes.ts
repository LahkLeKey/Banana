...
function bananaSignalScore(content: string):
    {score: number; matched: string[]; threshold: number;} {
  const tokens = content.toLowerCase().match(/[a-z0-9]+/g) ?? [];
  const bananaTokens = new Set(
      ['banana', 'ripe', 'peel', 'smoothie', 'fruit', 'harvest', 'bunch']);
  const notBananaTokens = new Set([
    'plastic', 'battery', 'engine', 'asphalt', 'steel', 'concrete', 'garbage'
  ]);
  const matchedBanana = tokens.filter((token) => bananaTokens.has(token));
  const matchedNotBanana = tokens.filter((token) => notBananaTokens.has(token));
  const matched = [...new Set([...matchedBanana, ...matchedNotBanana])];
  const bananaHits = matchedBanana.length;
  const notBananaHits = matchedNotBanana.length;
  const totalHits = bananaHits + notBananaHits;
  ...
  return {
    score: clamp01(bananaHits / totalHits),
    matched,
    threshold: 0.6,
  };
}
...
function buildAssistantReply(content: string): string {
  ...
  if (signal.score >= positiveThreshold) {
    ...
    return `banana-forward signal detected (${matched}) with confidence ${
        signal.score.toFixed(2)} (threshold=${positiveThreshold.toFixed(2)})`;
  }

  if (signal.score <= negativeThreshold) {
    return `banana-light signal is stronger right now (score=${
  ...
  return `mixed signal detected (banana_score=${
      signal.score.toFixed(2)}, threshold=${
      positiveThreshold.toFixed(
          2)}); ask a follow-up for clearer banana context`;
}

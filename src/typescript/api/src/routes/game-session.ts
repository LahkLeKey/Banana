...
export interface ValidationResult {
    readonly ok: boolean;
    readonly reason
        ?:|"sequence_non_monotonic"|"tick_non_monotonic"|"invalid_axis"|"invalid_timestamp";
}

type SessionRecord = {
    readonly sessionId: string; readonly token : string; readonly playerName : string; lastTick : number; lastSequence : number; readonly createdAt :
                                                                                                                                              number;
    heartbeatLayers : {
...
function getSessionOrReply(sessionId: string,
...
export function validateInputCommandEnvelope(command: GameInputCommandEnvelope, previous
                                             ?: {sequence: number; tick : number}): ValidationResult
{
...
export async function registerGameSessionRoutes(
    app: FastifyInstance, antiCheatAdapter: AntiCheatInteropAdapter =
                              createDefaultAntiCheatInteropAdapter()): Promise<void>
{
    ...
    app.post("/api/game/session/heartbeat/transport", async (request, reply) => {
        ...
        session.heartbeatLayers.transport.lastSequence =
            Math.max(session.heartbeatLayers.transport.lastSequence, heartbeatSequence);
    ...
    app.post("/api/game/session/heartbeat/usermode", async (request, reply) => {
        ...
        session.heartbeatLayers.session.lastSequence =
            Math.max(session.heartbeatLayers.session.lastSequence, heartbeatSequence);
        session.heartbeatLayers.integrity.lastSequence =
            Math.max(session.heartbeatLayers.integrity.lastSequence, heartbeatSequence);
    ...
    app.get("/api/game/session/status", async (request, reply) => {
        const sessionId =
            ((request.query as {sessionId?: string}).sessionId ?? "").toString().trim();
        if (!sessionId)
        {
            return reply.status(400).send({error : "invalid_argument"});
        }

      ...
      {cells.map((cell, index) => {
        ...
        return (
          ...
          >
            ...
            <div
              className={`flex w-12 flex-none items-start justify-end py-2.5 pr-2.5 font-mono text-[11px] tabular-nums transition-colors ${
                isActive
            ...
            <div className="min-w-0 flex-1 py-2 pr-3">
              <div className="flex items-center gap-1.5">
                <span
                  className={`text-[11px] font-medium ${isActive ? "text-blue-700" : "text-slate-500"}`}
                >
                  {index + 1}
                </span>
                <span
                  className={`rounded px-1 text-[10px] ${isActive ? "bg-blue-100 text-blue-600" : "bg-slate-100 text-slate-500"}`}
                >
                  {cell.kind}
                </span>
                {cell.running && (
                  <span className="animate-pulse rounded bg-amber-100 px-1 text-[10px] font-medium text-amber-600">
                    •
                  </span>
                )}
              ...
              <p
                className={`mt-0.5 truncate font-mono text-[11px] ${isActive ? "text-slate-700" : "text-slate-500"}`}
              >

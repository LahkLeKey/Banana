          ...
          <RailSection label="Runtime Contract Map">
            <div className="grid gap-1.5 md:grid-cols-2 xl:grid-cols-3">
              {contracts.map((c) => (
                <div
                  key={c.marker}
                  className={`rounded border px-2.5 py-2 ${CONTRACT_TONE_LIGHT[c.status]}`}
                >
                  <div className="flex items-start justify-between gap-1">
                    <p className="text-xs font-semibold">{c.title}</p>
                    <span
                      className={`mt-0.5 shrink-0 rounded border px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${CONTRACT_PILL_TONE[c.status]}`}
                    >
                      {c.status}
                    </span>
                  </div>

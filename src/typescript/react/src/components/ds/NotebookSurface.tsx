    ...
    <SurfaceShell family={family} title={title} summary={summary}>
      <div className="space-y-3">
        <svg
          ...
          className="w-full rounded border border-white/10 bg-zinc-950/70"
        >
          <line
            x1={margin.left}
              ...
              return (
                ...
                >
                  <rect
                    ...
                    height={margin.top + chartHeight - point.y}
                    rx="6"
                    fill={hoverIndex === index ? "#f8fafc" : accent}
                    opacity={hoverIndex === index ? 1 : 0.88}
                  />
                </g>
              );
            })}
          {(mode === "line" || mode === "scatter") &&
            coords.map((point, index) => (
              <g
                key={point.x}
                onMouseEnter={() => setHoverIndex(index)}
              ...
              >
                <circle
                  ...
                  fill={hoverIndex === index ? "#ffffff" : accent}
                />
              </g>
            ))}
          {coords.map((point) => (
        ...
        <div className="grid gap-2 sm:grid-cols-3">
          {coords.map((point, index) => (
            <button
              ...
              onMouseEnter={() => setHoverIndex(index)}
              onMouseLeave={() => setHoverIndex(null)}
              className={`rounded border px-3 py-2 text-left text-xs transition-colors ${hoverIndex === index ? "border-white/40 bg-white/10 text-white" : "border-white/10 bg-black/10 text-zinc-300"}`}
            >
...
function WidgetSurface({
  ...
  const datasets =
    payload.datasets && typeof payload.datasets === "object" && !Array.isArray(payload.datasets)
  ...
  return (
    ...
    >
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_180px]">
        <svg
          viewBox="0 0 100 70"
          className="w-full rounded border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(163,230,53,0.12),_transparent_50%),linear-gradient(180deg,rgba(10,15,10,0.95),rgba(12,20,12,1))]"
        >
            ...
            {points.map((point, index) => (
              <g key={point.label} onClick={() => setSelected(index)} className="cursor-pointer">
                <circle
                  cx={point.x}
                  cy={point.y}
                  r={selected === index ? 3.8 : 2.6}
                  fill={point.color ?? (selected === index ? "#fef08a" : "#bef264")}
                />
                <text
                  x={point.x + 2.5}
                  y={point.y - 2.5}
        ...
        <div className="space-y-2">
          {points.map((point, index) => (
            <button
              ...
              className={`w-full rounded border px-3 py-2 text-left text-xs ${selected === index ? "border-lime-300/50 bg-lime-200/10 text-white" : "border-white/10 bg-black/10 text-zinc-300"}`}
            >
              <span className="block font-semibold">{point.label}</span>
              <span className="block opacity-70">
                {point.value ?? Math.round(point.x + point.y)} units
              </span>
            </button>
          ))}
          {active && (
            <p className="rounded border border-white/10 bg-black/10 px-3 py-2 text-xs text-zinc-300">
              Focused marker: <span className="font-semibold text-white">{active.label}</span>
            </p>
          )}
        </div>
      </div>
          ...
          {edges.map((edge, index) => {
            ...
            return (
              <line
                ...
                x2={target.x}
                y2={target.y}
                stroke={active ? "#7dd3fc" : "rgba(255,255,255,0.18)"}
          ...
          {nodes.map((node) => (
            <g key={node.id} onClick={() => setActiveId(node.id)} className="cursor-pointer">
              <circle
                cx={node.x}
                cy={node.y}
                r={linkedIds.has(node.id) || node.id === activeId ? 16 : 12}
                fill={
                  node.id === activeId ? "#38bdf8" : linkedIds.has(node.id) ? "#7dd3fc" : "#1f2937"
                }
                stroke="rgba(255,255,255,0.4)"
              />
              <text x={node.x} y={node.y + 4} textAnchor="middle" fill="white" fontSize="10">
        ...
        <div className="space-y-2">
          {nodes.map((node) => (
            <button
              ...
              className={`w-full rounded border px-3 py-2 text-left text-xs ${node.id === activeId ? "border-sky-300/50 bg-sky-300/10 text-white" : "border-white/10 bg-black/10 text-zinc-300"}`}
            >
              {node.label}
            </button>
          ))}
...
function ThreeDSurface({
  ...
  const [speed, setSpeed] = useState(
    typeof payload.rotationSpeed === "number" ? payload.rotationSpeed : 0.02
  );
...
function CanvasSurface({
  ...
  const [amplitude, setAmplitude] = useState(
    typeof payload.amplitude === "number" ? payload.amplitude : 0.7
  );
  const [frequency, setFrequency] = useState(
    typeof payload.frequency === "number" ? payload.frequency : 2.4
  );
...
export function NotebookSurface({ surface }: { surface: NotebookSurfaceOutput }) {
  if (surface.family === "vega" || surface.family === "altair") {
    const spec =
      surface.payload.spec && typeof surface.payload.spec === "object"
    ...
    return (
      <ChartSurface
        ...
        points={points}
        mode={mark}
        accent={surface.family === "vega" ? "#22d3ee" : "#34d399"}
      />
    );
  }
  ...
  if (surface.family === "widgets") {
    return (
      <WidgetSurface title={surface.title} summary={surface.summary} payload={surface.payload} />
    );
  }
  ...
  if (surface.family === "bqplot") {
    return (
      <BqplotSurface title={surface.title} summary={surface.summary} payload={surface.payload} />
    );
  }
  ...
  if (surface.family === "network") {
    return (
      <NetworkSurface title={surface.title} summary={surface.summary} payload={surface.payload} />
    );
  }

  if (surface.family === "three-d") {
    return (
      <ThreeDSurface title={surface.title} summary={surface.summary} payload={surface.payload} />
    );
  }

  return (
    <CanvasSurface title={surface.title} summary={surface.summary} payload={surface.payload} />
  );
}

...
function RichArtifact({ out, index }: { out: NotebookRichOutput; index: number }) {
  ...
  if (out.kind === "image") {
    const src = out.data.startsWith("data:") ? out.data : `data:${out.mimeType};base64,${out.data}`;
    return (
          ...
          <thead className="sticky top-0 bg-zinc-900">
            <tr>
              {out.table.columns.map((c) => (
                <th
                  key={c}
                  className="border border-zinc-700 px-2 py-1 text-left font-semibold text-zinc-300"
                >
                  {c}
                </th>
              ))}
          ...
          <tbody>
            {out.table.rows.map((row, ridx) => (
              <tr key={ridx} className="even:bg-zinc-900">
                {row.map((val, cidx) => (
                  <td key={cidx} className="border border-zinc-800 px-2 py-1 text-zinc-300">
                    {String(val ?? "")}
                  </td>
                ))}
              </tr>
    ...
    return (
      <div key={`error-${index}`} className="border-b border-zinc-800 bg-red-950/30 px-4 py-3">
        <div className="mb-1 flex items-center gap-2">
          <span className="rounded bg-red-900/60 px-1.5 py-0.5 font-mono text-[11px] font-semibold text-red-300">
            {out.ename}
          </span>
          <span className="text-xs text-red-400">{out.evalue}</span>
        </div>
        <pre className="max-h-48 overflow-auto whitespace-pre-wrap font-mono text-[11px] leading-5 text-red-400/80">
          {out.traceback}
        </pre>
      </div>
    );
      ...
      <div key={`contract-${index}`} className="border-b border-zinc-800 px-4 py-3">
        <div className={`rounded border px-3 py-3 ${CONTRACT_TONE_DARK[out.status]}`}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <span
              className={`rounded border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${CONTRACT_PILL_TONE[out.status]}`}
            >
              {out.status}
            </span>
          </div>
          ...
          {out.payloadText && (
            <details className="mt-3 rounded border border-white/10 bg-black/10">
              ...
              <pre className="max-h-56 overflow-auto border-t border-white/10 px-2 py-2 font-mono text-[11px] leading-5 opacity-90">
                {out.payloadText}
              </pre>
            </details>
          )}
  ...
  return (
    <div
      key={`notice-${index}`}
      className={`border-b border-zinc-800 px-4 py-2 font-mono text-xs ${out.level === "warning" ? "text-amber-300" : "text-zinc-400"}`}
    >
      {out.message}
    </div>
  );
  ...
  return (
    <div className="border-t border-zinc-800 bg-zinc-950 text-white">
      <div className="flex items-center justify-between border-b border-zinc-800 px-4 py-2">
        <span className="font-mono text-[11px] text-zinc-500">Out [{execCount ?? " "}]:</span>
        {richOutput && richOutput.length > 0 && (
          <span className="text-[11px] text-zinc-500">
            {richOutput.length} artifact{richOutput.length !== 1 ? "s" : ""}
          </span>
        )}
      </div>

      {richOutput?.map((out, i) => (
        <RichArtifact key={i} out={out} index={i} />
      ))}

      {output && (
        <pre className="max-h-64 overflow-auto whitespace-pre-wrap px-4 py-3 font-mono text-xs text-zinc-300">
          {output}
        </pre>
      )}
      {error && (
        <pre className="max-h-48 overflow-auto whitespace-pre-wrap bg-red-950/40 px-4 py-3 font-mono text-xs text-red-400">
          {error}
        </pre>
      )}
    </div>

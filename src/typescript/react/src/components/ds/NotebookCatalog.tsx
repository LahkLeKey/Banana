  ...
  return (
    <aside className="flex w-full min-w-0 flex-col rounded-lg border border-slate-200 bg-white">
      ...
      <div className="max-h-44 overflow-y-auto py-1">
        {library.map((entry) => {
          ...
          return (
            ...
            >
              <span
                className={`block truncate text-xs ${isActive ? "font-semibold" : "font-medium"}`}
              >
                {entry.label}
              </span>
      ...
      <div className="space-y-1 border-t border-slate-200 p-2">
        <p className="px-1 pb-0.5 text-[10px] font-semibold uppercase tracking-wider text-slate-400">
          Add Cell
        </p>
        <Button

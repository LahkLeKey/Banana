...
import { wasmStatusMessage, wasmAllocString } from './wasmRuntime';
...
export function classifyBanaryBinary(
  mod: BananaWasmModule,
  inputJson: string,
...
export function classifyBanaryBinaryWithThreshold(
  mod: BananaWasmModule,
  inputJson: string,
  threshold: number,
): WasmResult<BinaryClassifyResult> {
  ...
  try {
    const status = mod._banana_classify_banana_binary_with_threshold(inputPtr, threshold, outPtrPtr);
    if (status !== 0) return { ok: false, code: status, message: wasmStatusMessage(status) };
    const jsonPtr = mod.getValue(outPtrPtr, 'i32');
    const jsonStr = mod.UTF8ToString(jsonPtr);
    mod._banana_free(jsonPtr);

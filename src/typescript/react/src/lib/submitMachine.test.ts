...
import {initialSubmitState, type SubmitAction, submitReducer, type SubmitState,} from './submitMachine';
...
describe('submitReducer (spec 048)', () => {
  test('starts in idle with null lastResult', () => {
    ...
    expect((init as Extract<SubmitState<Verdict>, {kind: 'idle'}>).lastResult)
        .toBeNull();
  });

  test(
      'idle -> submit -> submitting carries requestId and null lastResult',
      () => {
  ...
  test('submitting -> resolve (matching id) -> success', () => {
    const submitting = submitReducer(
        initialSubmitState<Verdict>(), {type: 'submit', requestId: 'r1'});
    const next = submitReducer(
        submitting,
        {type: 'resolve', requestId: 'r1', result: {label: 'banana'}});
  ...
  test(
      'submitting -> reject (matching id) -> error preserves lastResult',
      () => {
        ...
        state = submitReducer(
            state, {type: 'reject', requestId: 'r2', message: 'boom'});
  ...
  test('stale resolve is ignored (non-matching requestId)', () => {
    let state: SubmitState<Verdict> = initialSubmitState();
    state = submitReducer(state, {type: 'submit', requestId: 'r1'});
    const stale = submitReducer(
        state, {type: 'resolve', requestId: 'OLD', result: {label: 'x'}});
  ...
  test('stale reject is ignored (non-matching requestId)', () => {
    ...
    state = submitReducer(state, {type: 'submit', requestId: 'r1'});
    const stale = submitReducer(
        state, {type: 'reject', requestId: 'OLD', message: 'boom'});
    expect(stale).toBe(state);
  });
  ...
  test('success -> submit carries lastResult forward', () => {
    ...
    state = submitReducer(state, {type: 'submit', requestId: 'r1'});
    state = submitReducer(
        state, {type: 'resolve', requestId: 'r1', result: {label: 'banana'}});
    state = submitReducer(state, {type: 'submit', requestId: 'r2'});

...
export function analyzeQuizBenchmark(
  attempts: QuizAttempt[],
  options: QuizBenchmarkOptions = {}
): QuizBenchmarkSummary {
  ...
  const orderedAttempts = [...attempts].sort((a, b) =>
    a.answeredAtUtc.localeCompare(b.answeredAtUtc)
  );
  ...
  if (currentAverage < regressionFloor) {
    ...
    currentAverage >= benchmarkTarget &&

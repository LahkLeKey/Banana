...
describe("react api client", () => {
    ...
    test("resolves chat API base with localhost fastify fallback", () => {
        ...
        expect(resolveChatApiBaseUrl("http://localhost:8080", "http://localhost:9081", undefined))
    ...
    test("sendChatMessage posts content and idempotency key", async () => {
        ...
        const payload =
            await sendChatMessage("http://api.example", "chat_1", "banana question", "client-1");
    ...
    test("propagates API errors with typed message", async () => {
        globalThis.fetch =
            (async () => new Response(JSON.stringify({error : {message : "banana backend down"}}), {
                 status : 503,
                 headers : {"content-type" : "application/json"},
             })) as typeof fetch;
    ...
    test("ingestTelemetryEvent posts the telemetry payload", async () => {
        ...
        globalThis.fetch =
            (async (input: RequestInfo|URL, init?: RequestInit) => {
                lastUrl = String(input);
                lastInit = init;
                return new Response(JSON.stringify({persisted : true, backend : "memory"}), {
                    status : 202,
                    headers : {"content-type" : "application/json"},
                });
            }) as typeof fetch;
    ...
    test("fetchEnsembleVerdict snapshots the slice 014 contract shape", async () => {
        ...
        const verdict =
            await fetchEnsembleVerdict("http://api.example", "yellow fruit on the counter maybe");
    ...
    test("fetchEnsembleVerdictWithEmbedding returns verdict and 4-dim embedding", async () => {
        ...
        const result = await fetchEnsembleVerdictWithEmbedding("http://api.example",
    ...
    test("fetchEnsembleVerdictWithEmbedding accepts a null embedding (cheap-path verdict)",
         async () => {
             globalThis.fetch =
                 (async () => new Response(JSON.stringify({
                      verdict : {
             ...
             const result =
                 await fetchEnsembleVerdictWithEmbedding("http://api.example", "ripe banana");
    ...
    test("fetchEnsembleVerdictWithEmbedding propagates 5xx errors", async () => {
        ...
        await expect(fetchEnsembleVerdictWithEmbedding("http://api.example", "ambiguous"))

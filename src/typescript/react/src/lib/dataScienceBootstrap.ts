...
export const DATA_SCIENCE_FOLLOW_UP_SPIKES: DataScienceSpike[] = [
  ...
  },
  {
    ...
    focus: "Capture Swagger Try-It-Out execution and replay the exact request in a notebook cell.",
  },
  {
    id: "DS-224",
...
export function extractEndpointOperations(document: unknown): EndpointOperation[] {
  ...
  for (const [path, methods] of Object.entries(openApi.paths)) {
    for (const [method, details] of Object.entries(methods ?? {})) {
      ...
      if (
        !["GET", "POST", "PUT", "PATCH", "DELETE", "HEAD", "OPTIONS"].includes(normalizedMethod)
      ) {
        continue;
      }
...
export function buildEndpointBootstrapCells(
  operations: EndpointOperation[],
  apiBaseUrl: string
): BootstrapCell[] {
  const safeBase = apiBaseUrl.replace(/\/$/, "");
  const notebookOperations = operations.map((op) => ({
    method: op.method,
    path: op.path,
  ...
  const safeGetCandidates = notebookOperations

...
function reducer(state: WorkerHookState, action: WorkerAction): WorkerHookState {
  switch (action.type) {
    case 'QUEUED':    return { ...state, state: 'queued', result: null, error: null };
    case 'INIT':      return { ...state, state: 'loading' };
    case 'EXECUTING': return { ...state, state: 'loading' };
    case 'RESULT':    return { state: 'ready', result: action.data, error: null };
    case 'ERROR':     return { state: 'error', result: null, error: { code: action.code, message: action.message } };
    case 'FALLBACK':  return { ...state, state: 'degraded', error: null };
    case 'RESET':     return { state: 'idle', result: null, error: null };
    default:          return state;
  }
}

const INITIAL: WorkerHookState = { state: 'idle', result: null, error: null };

// ── Worker factory (singleton per hook instance) ──────────────────────────────
function createWorker(): Worker {
  return new Worker(new URL('../workers/bananaWasmWorker.ts', import.meta.url), { type: 'module' });
}
  ...
  function getWorker(): Worker {
    if (!workerRef.current) {
      ...
      workerRef.current.onmessage = (evt: MessageEvent) => {
        const msg = evt.data as { type: string; id?: string; [k: string]: unknown };
        ...
        switch (msg.type) {
          ...
          case 'error':
            pendingIdRef.current = null;
            dispatch({ type: 'ERROR', code: msg.code as number, message: msg.message as string });
            break;
          case 'fallback':
            pendingIdRef.current = null;

...
export function renderMarkdown(src: string): string {
  return src
    ...
    .map((line) => {
      ...
      if (/^####\s/.test(line)) return `<h4 class="text-sm font-bold mt-3">${line.slice(5)}</h4>`;
      if (/^###\s/.test(line)) return `<h3 class="text-base font-bold mt-3">${line.slice(4)}</h3>`;
      if (/^##\s/.test(line)) return `<h2 class="text-lg font-bold mt-4">${line.slice(3)}</h2>`;
      if (/^#\s/.test(line)) return `<h1 class="text-xl font-bold mt-4">${line.slice(2)}</h1>`;
      ...
      return `<p class="leading-relaxed">${line
        ...
        .replace(
          /`(.+?)`/g,
          '<code class="rounded bg-muted px-1 font-mono text-xs">$1</code>'
        )}</p>`;
    })
    .join("\n");
}
...
export function normalizeRichOutput(raw: unknown): NotebookRichOutput[] | undefined {
  ...
  for (const item of raw) {
      ...
      if (fig && Array.isArray(fig.data)) {
        normalized.push({
          ...
          figure: {
            data: fig.data,
            layout:
              typeof fig.layout === "object" && fig.layout
      ...
      if (Array.isArray(columns) && Array.isArray(rows)) {
        normalized.push({
          ...
          table: {
            ...
            rows: rows.map((row) =>
              Array.isArray(row)
                ? row.map((v) => (v == null ? null : typeof v === "number" ? v : String(v)))
                : []
            ),
          },
        });
    ...
    if (kind === "notice") {
      const message =
        typeof entry.message === "string" ? entry.message : "Unsupported visualization payload.";
    ...
    if (kind === "surface") {
      const family =
        typeof entry.family === "string" ? (entry.family as RuntimeContractFamily) : undefined;
      const title = typeof entry.title === "string" ? entry.title : undefined;
      const payload = entry.payload;
      if (
        family &&
        LIVE_SURFACE_FAMILIES.has(family) &&
        title &&
        payload &&
    ...
    if (kind === "contract") {
      ...
      if (family && status && title && summary && marker) {
        normalized.push({
          ...
          marker,
          payloadText: typeof entry.payloadText === "string" ? entry.payloadText : undefined,
        });
      }
    }
  }
    ...
    if (out.output_type === "stream") {
      const txt = Array.isArray(out.text)
        ...
        : typeof out.text === "string"
          ? out.text
          : "";
      if (txt.trim()) output = output ? `${output}\n${txt}` : txt;
    }
...
export function normalizeNotebookForBrowser(cells: NotebookCell[]): NotebookCell[] {
  return cells.map((cell) => {
    ...
    if (cell.kind !== "python") return baseCell;

    const src = cell.source;
    const looksLikeLegacyHelper =
      src.includes("urllib.request") &&
      src.includes("def call_endpoint(") &&
      src.includes("API_BASE");
    if (!looksLikeLegacyHelper) return baseCell;
...
export function withNotebookDefaults(cells: NotebookCell[]): NotebookCell[] {
  return normalizeNotebookForBrowser(cells).map((cell) => ({
    ...cell,
    previewMode: cell.kind === "markdown" ? (cell.previewMode ?? true) : undefined,
  }));
}
...
function downsampleTrace(trace: Record<string, unknown>): {
  ...
  const x = trace.x;
  const y = trace.y;
  if (!Array.isArray(x) || !Array.isArray(y)) return { trace, reduced: false };
  const n = Math.min(x.length, y.length);
  if (n <= PLOTLY_MAX_TRACE_POINTS) return { trace, reduced: false };
...
export function normalizePlotlyFigure(raw: unknown): { figure?: PlotlyFigure; notices: string[] } {
  ...
  if (!Array.isArray(obj.data)) return { notices: ["Plotly payload ignored: missing data array."] };
  ...
  for (const traceRaw of obj.data) {
    ...
    if (result.reduced) reducedAny = true;
    normalizedData.push(result.trace);
  }

  if (normalizedData.length === 0) return { notices: ["Plotly payload ignored: no valid traces."] };
  if (reducedAny)
    notices.push(
      `Plotly traces were downsampled to ${
        PLOTLY_MAX_TRACE_POINTS
      } points for notebook responsiveness.`
    );
  ...
  return {
    figure: {
      data: normalizedData,
      layout:
        typeof obj.layout === "object" && obj.layout
...
export function buildContractCard(
  definition: RuntimeContractDefinition,
  rawPayload: string
): RuntimeContractCard {
  ...
  const status =
    payload?.status === "active" || payload?.status === "gated" || payload?.status === "deferred"
      ? payload.status
      : definition.status;
  return {
    kind: "contract",
...
function buildSurfaceOutput(
  ...
  const title = typeof payload.title === "string" ? payload.title : definition.title;
  const summary = typeof payload.summary === "string" ? payload.summary : definition.summary;
  const { title: _title, summary: _summary, status: _status, detail: _detail, ...rest } = payload;
  ...
  for (const line of lines) {
    if (line.startsWith("BANANA_PLOTLY::")) {
      try {
        ...
        if (normalized.figure) richOutputs.push({ kind: "plotly", figure: normalized.figure });
    ...
    for (const contract of RUNTIME_CONTRACTS) {
      if (contract.marker === "BANANA_PLOTLY::" || contract.marker === "BANANA_TABLE::") continue;

...
const ARTICLES: Article[] = [
  // Platform Engineering
  {
    ...
    summary:
      "Wrapper ABI surface, status codes, buffer ownership, and downstream interop contracts.",
    detail:
      "Covers all exported C symbols, status code taxonomy, buffer lifetime rules, and how ASP.NET P/Invoke consumes the wrapper. Update whenever an export is added or renamed.",
  ...
  },
  {
    ...
    summary:
      "CMakePresets.json configure targets, sanitizer lanes, and cross-platform build matrix.",
    detail:
      "Documents each named preset (debug, release, asan, ci), the output path contract (/out/native/bin), and the Docker native-builder stage dependencies.",
  ...
  },
  {
    ...
    detail:
      "Ordered pipeline steps (InputValidation:10, DriftSampling:50, EnsembleGating:100, Escalation:200, Calibration:300, AbVariant:150). New steps must register with a deterministic Order value and integration tests for ordering behavior.",
  ...
  },
  {
    ...
    detail:
      "All React, Electron, and React Native apps must consume @banana/ui tokens rather than inline values. Covers token generation, web/native split, and the Storybook documentation surface.",
  ...
  // Model Runtime
  {
    ...
    detail:
      "The full ensemble flow: gating step checks confidence threshold, escalation promotes low-confidence verdicts, calibration applies Platt scaling, and A/B variant step tags requests for experiment tracking.",
  ...
  },
  {
    ...
    detail:
      "train-banana-model.py, train-ripeness-model.py, and train-not-banana-model.py each accept --training-profile ci|local and --session-mode single|multi. CI profiles enforce max-sessions 1 for deterministic runs.",
  ...
  },
  {
    ...
    detail:
      "Drift > 0.1 cosine distance signals significant distribution shift. Computed in @banana/ui computeCosineDrift and surfaced on the Knowledge page. Feeds the Spike 078 drift-detection flow.",
  ...
  },
  {
    ...
    detail:
      "Models land in artifacts/training/<model>/local for local runs and .artifacts/<model> for CI. The release flow signs artifacts before promotion. BANANA_NATIVE_PATH must reference the correct .so path at runtime.",
  ...
  // Delivery Contracts
  {
    ...
    summary:
      "Release readiness evidence linked to runtime contracts, rollout checks, and post-deploy health thresholds.",
    detail:
      "Covers pre-deploy checklist, canary health gate, post-deploy /health assertion, and rollback trigger conditions. Canonical source for go/no-go decisions.",
  ...
  },
  {
    ...
    detail:
      "Production builds require VITE_BANANA_API_BASE_URL=https://api.banana.engineer. vercel --yes --prod deploys from the React dist output. Preview deploys use BANANA_CORS_ORIGINS env var expansion on the API side.",
  ...
  // Incident Playbooks
  {
    ...
    summary:
      "Container-driven local runtime: Docker Desktop + Ubuntu WSL2 + Electron/Mobile channels.",
    detail:
      "Launch via scripts/launch-container-channels-with-wsl2-electron.sh from Windows shell. Requires Docker Desktop + Ubuntu-24.04 WSL2. Exit code 42 = integration preflight failure with actionable remediation.",
  ...
  },
  {
    ...
    summary:
      "Swashbuckle schema ID collision fix, CORS middleware ordering, and spec endpoint health.",
    detail:
      "Root cause: nested InputJsonRequest types across controllers collide on schemaId. Fix: CustomSchemaIds(type => type.FullName). CORS must be registered before UseSwagger. Validate with curl -i /swagger/v1/swagger.json post-deploy.",
...
export function KnowledgePage() {
  ...
  return (
    <div className="space-y-4">
      {/* Health strip */}
      <div
        className={`flex items-center gap-3 rounded-md border px-4 py-2 text-sm ${healthStatus === "ok" ? "border-green-500/40 bg-green-500/5 text-green-700" : healthStatus === "degraded" ? "border-yellow-500/40 bg-yellow-500/5 text-yellow-700" : healthStatus === "unavailable" ? "border-red-500/40 bg-red-500/5 text-red-700" : "border-border text-muted-foreground"}`}
      >
        <span
          className={`inline-block h-2 w-2 rounded-full ${healthStatus === "ok" ? "bg-green-500" : healthStatus === "degraded" ? "bg-yellow-500" : healthStatus === "unavailable" ? "bg-red-500" : "bg-muted-foreground/40 animate-pulse"}`}
        />
          ...
          {filteredArticles.map((a) => (
            <Card key={a.title}>
              <CardHeader className="pb-2">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <CardTitle className="text-base">
                      <a href={a.href} target="_blank" rel="noreferrer" className="hover:underline">
                        {a.title}
                      </a>
                    </CardTitle>
                ...
                <span className="inline-block rounded-full border px-2 py-0.5 text-[10px] text-muted-foreground">
                  {a.space}
                </span>
              </CardHeader>
              {expandedArticle === a.title && (
                <CardContent className="pt-0">
                  <p className="rounded-md bg-muted/40 px-3 py-2 text-sm text-foreground/80">
                    {a.detail}
                  </p>
                </CardContent>
              )}
          ...
          {rows.length > 0 && (
            <>
              <div className="flex items-center justify-between px-1">
                ...
                <Button size="sm" variant="outline" onClick={copyVisibleMetrics}>
                  Copy/Export Metrics
                </Button>
              </div>
                ...
                <CardContent className="space-y-2 text-sm">
                  ...
                  {selectedRun && (
                    <div className="rounded-md border bg-muted/30 p-3 text-xs space-y-1">
                      <p>
                        <span className="text-muted-foreground">run_id</span> {selectedRun.run_id}
                      </p>
                      <p>
                        <span className="text-muted-foreground">lane</span> {selectedRun.lane}
                      </p>
                      <p>
        ...
        <div className="space-y-3">
          <Card>
            ...
            <CardContent className="space-y-2 text-xs text-muted-foreground">
              {recentRuns.length > 0 ? (
                recentRuns.map((run: TrainingRunResult) => (
                  <div key={run.run_id} className="border-b pb-1 last:border-0">
                    <div className="flex items-center justify-between gap-2">
                      <p className="font-medium text-foreground">{run.lane}</p>
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] ${laneHealth(run.lane) === "healthy" ? "border-green-500 text-green-600" : laneHealth(run.lane) === "warning" ? "border-yellow-500 text-yellow-600" : "border-red-500 text-red-600"}`}
                      >
                        {laneHealth(run.lane)}
                      </span>
                    </div>
                    <p>
                      {run.status} · {run.run_id.slice(-8)}
                    </p>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-xs">
              {driftByLane.map(({ lane, drift }) => (
                <div key={lane} className="flex items-center justify-between">
                  ...
                  {drift == null ? (
                    ...
                    <span
                      className={drift > 0.1 ? "text-yellow-600 font-medium" : "text-green-600"}
                    >
                      Δ{drift.toFixed(4)}
                    </span>
                  )}
              ...
              <p className="text-muted-foreground/50 pt-1">
                cosine dist between last 2 run centroids
              </p>
            </CardContent>
          </Card>
        </div>

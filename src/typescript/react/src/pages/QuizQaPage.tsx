...
export function QuizQaPage() {
  ...
  function addMockAttempt() {
    ...
    const mock: QuizAttemptRow = {
      ...
      expectedLabel,
      predictedLabel:
        autoScore === 1 ? expectedLabel : expectedLabel === "banana" ? "not-banana" : "banana",
      graderNotes: "",
    };
    ...
    <div className="space-y-4" data-testid="quiz-qa-page">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Run repeatable quiz checks against the model, track benchmark trend, and grade past
            answers with a human-in-the-loop workflow.
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <Card data-testid="quiz-benchmark-card">
        <CardHeader>
          ...
          <CardDescription>
            Target benchmark: {(benchmarkSummary.benchmarkTarget * 100).toFixed(0)}% · Regression
            floor: {(benchmarkSummary.regressionFloor * 100).toFixed(0)}%
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            ...
            <div className="rounded border px-3 py-2" data-testid="quiz-previous-score">
              Previous average:{" "}
              {benchmarkSummary.previousAverage == null
                ? "n/a"
                : `${(benchmarkSummary.previousAverage * 100).toFixed(2)}%`}
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <label className="text-sm" htmlFor="quiz-question-select">
            Quiz question
          </label>
          <select
          ...
          >
            {quizQuestions.map((question) => (
              <option key={question.id} value={question.id}>
                {question.id} · {question.expectedLabel}
              </option>
            ))}
          </select>
          ...
          <label className="text-sm" htmlFor="quiz-expected-label">
            Expected label
          </label>
          <select
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              data-testid="quiz-run-model-btn"
              onClick={runQuizAgainstModel}
              disabled={runningQuiz || !questionText.trim()}
            >
            ...
            <Button
              type="button"
              variant="outline"
              data-testid="quiz-add-mock-btn"
              onClick={addMockAttempt}
            >
              Add Mock Attempt
            </Button>
          </div>
          ...
          {runError ? (
            <p className="text-sm text-destructive" data-testid="quiz-run-error">
              {runError}
            </p>
          ) : null}
        </CardContent>
          ...
          {history.length === 0 ? (
            ...
            history.map((item) => (
              <div
                ...
                data-testid="quiz-history-item"
              >
                <div className="grid gap-1 text-sm md:grid-cols-2">
                  <p>
                    <span className="font-medium">Expected:</span> {item.expectedLabel}
                  </p>
                  <p>
                    <span className="font-medium">Predicted:</span> {item.predictedLabel}
                  </p>
                  <p>
                    <span className="font-medium">Auto score:</span>{" "}
                    {(item.autoScore * 100).toFixed(0)}%
                  </p>
                  <p>
                    <span className="font-medium">Answered:</span> {item.answeredAtUtc}
                  </p>
                </div>
                ...
                <div className="grid gap-2 md:grid-cols-[180px_1fr]">
                  <select
                    ...
                    onChange={(event) =>
                      updateHumanGrade(item.id, event.target.value as HumanGrade)
                    }

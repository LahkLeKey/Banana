...
import { MobileRadialControl } from "@banana/ui";
import { ContextMenu } from "@banana/ui";
import {
  SplashOverlay,
...
import { NetcodeDomain, ReplicationDomain } from "@banana/ui";
  ...
  useEffect(() => {
    ...
    const startEngine = async () => {
      ...
      interactionMgr.registerActionHandler("interact", (actionId, normalizedX, normalizedY) => {
        const mod = moduleRef.current;
        if (!mod) {
          return { actionId, targetNormalizedX: normalizedX, targetNormalizedY: normalizedY, success: false, message: "Engine not ready", timestamp: Date.now() };
        }
        const result = Number(
          mod.ccall("engine_handle_right_click_normalized", "number", ["number", "number"], [
            normalizedX,
            normalizedY,
          ])
        );
        return {
          ...
          success: result === 1,
          message: result === 1 ? "Interact: applied to nearest actor." : "Interact: no target in range.",
          timestamp: Date.now(),
        };
      });
      ...
      interactionMgr.registerActionHandler("harvest", (actionId, normalizedX, normalizedY) => {
        ...
        return {
          ...
          targetNormalizedY: normalizedY,
          success: result === 1,
          message: result === 1 ? "Harvest: applied to nearest actor." : "Harvest: no target in range.",
          timestamp: Date.now(),
        };
      });
      ...
      const applyRuntimeFatal = (rawMessage: string) => {
        ...
        uiService.setEngineStatus("unavailable", "WebGL viewport initialization failed on this runtime. Open on a WebGL2-capable browser/device or enable hardware acceleration, then reload.");
      };
      ...
      try {
        ...
        const fpsLoop = () => {
          try {
            ...
            mod.ccall("engine_set_move_input", "null", ["number", "number"], [movement.moveX, movement.moveZ]);
            ...
            uiService.setEngineStatus("error", "Engine runtime failed while rendering. Reload the page or run on a browser/device with working WebGL2.");
            window.clearInterval(intervalRef.current);
          }
        };

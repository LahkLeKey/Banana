...
export function TelemetryDashboardPage({ autoHydrate = true }: { autoHydrate?: boolean }) {
      ...
      try {
        ...
        if (rootResult.status === "fulfilled") {
          ...
          emitTelemetry({
            ...
            details: {
              message:
                rootResult.reason instanceof Error
        ...
        const successCount = [rootResult, healthResult, configResult].filter(
        ...
        if (successCount > 0) {
          ...
          setHydration({
            status: "error",
            message:
              "API telemetry probe failed for all checks. Inspect runtime/API health and base URL.",
          });
        }
      } catch (error) {
        if (!cancelled) {
  ...
  const timelineBuckets = useMemo(
    () => buildBuckets(filteredEvents, rangePreset, now),
    [filteredEvents, rangePreset, now]
  );
    ...
    wasmEvents
      .filter(
        (event) =>
          ...
          (event.event === "wasm.worker.call.error" &&
            (event.details?.message ?? "") === "execution_timeout")
      )
      .map((event) => event.details?.id)
      .filter((id): id is string => typeof id === "string" && id.length > 0)
  );
  ...
  const latestEvents = [...filteredEvents].sort((a, b) => b.timestamp - a.timestamp).slice(0, 20);
    ...
    <div className="space-y-6" data-testid="telemetry-dashboard-page">
      ...
      <div className="grid gap-4 xl:grid-cols-[2fr,1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Layered traffic bars with per-source toggles and error overlays.
            </CardDescription>
          </CardHeader>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.4fr,1fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Most frequent event signatures in the selected window.
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <tbody>
                    {topEventSignatures.map((signal) => (
                      <tr key={signal.eventName} className="border-b last:border-b-0">
                        ...
                        <td className="py-2">
                          {new Date(signal.lastTimestamp).toLocaleTimeString()}
                        </td>
                      </tr>
                    ))}
                  </tbody>
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap gap-x-3 text-[10px] text-muted-foreground">
                      ...
                      {typeof event.durationMs === "number" ? (
                        <span>{Math.round(event.durationMs)} ms</span>
                      ) : null}
                    </div>
                  </div>
                ))}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Grouped fingerprints by source, event, code, and reason/message.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {errorFingerprints.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No error fingerprints for current filters.
            </p>
          ) : (
            <div className="overflow-x-auto">
                ...
                <tbody>
                  {errorFingerprints.map((fingerprint) => (
                    ...
                    >
                      ...
                      <td className="max-w-[360px] truncate py-2 pr-2" title={fingerprint.message}>
                        {fingerprint.message}
                      </td>
                      <td className="py-2 pr-2">{fingerprint.count}</td>
                      <td className="py-2">
                        {new Date(fingerprint.lastSeen).toLocaleTimeString()}
                      </td>
                    </tr>
                  ))}
                </tbody>

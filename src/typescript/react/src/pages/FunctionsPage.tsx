...
import {
  fetchTrainingWorkbenchHistory,
  predictRipeness,
  promoteTrainingWorkbenchRun,
  resolveApiBaseUrl,
  runTrainingWorkbench,
} from "../lib/api";
...
async function runRipenessSmokeFn(): Promise<FnActionResult> {
  const base = resolveApiBaseUrl();
  const data = await predictRipeness(base, {
    sample: "yellow banana, slightly soft, brown spots at tip",
  });
  return {
...
async function promoteLatestCandidateFn(): Promise<FnActionResult> {
  ...
  const result = await promoteTrainingWorkbenchRun(
    base,
    passed.run_id,
    "candidate",
    "suite-ui",
    "Promoted via Function Catalog governance gate"
  );
...
const CATALOG: FnGroup[] = [
  {
    ...
    fns: [
      ...
      {
        name: "Fetch Banana Summary",
        owner: "Classifier",
        status: "ready",
        action: fetchBananaSummaryFn,
      },
    ],
  },
  {
    ...
    fns: [
      ...
      },
      {
        ...
        confirm:
          "This will promote the latest passed training run to candidate. threshold_passed must be true. Proceed?",
        requiresWorkbench: true,
      },
    ],
  },
...
export function FunctionsPage() {
  ...
  async function run(fn: FnDef) {
    ...
    try {
      ...
      setStructuredError({
        ...
        message,
        remediation:
          "Confirm capability checks, validate API base URL, and retry with Dry-run enabled first.",
        source: fn.name,
      });
  ...
  const releaseReady =
    releaseChecklist.docs && releaseChecklist.tests && releaseChecklist.approvals && canaryHealthy;
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-2 text-sm">
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border px-2 py-1 text-xs">
              health capability: {capabilities.health ? "available" : "unavailable"}
            </span>
            <span className="rounded-full border px-2 py-1 text-xs">
              workbench capability: {capabilities.workbench ? "available" : "unavailable"}
            </span>
            <Button
              size="sm"
          ...
          {promotionAudit.length === 0 ? (
            ...
            promotionAudit.map((entry) => (
              <div key={`${entry.run_id}-${entry.promoted_at}`} className="rounded border p-2">
                <p>
                  <span className="text-muted-foreground">run</span> {entry.run_id}
                </p>
                <p>
                  <span className="text-muted-foreground">lane</span> {entry.lane}
                </p>
                <p>
                  <span className="text-muted-foreground">at</span> {entry.promoted_at}
                </p>
              </div>
            ))
          )}
      ...
      {structuredError && (
        <Card>
          ...
          <CardContent className="space-y-1 text-sm">
            <p>
              <span className="text-muted-foreground">code</span> {structuredError.code}
            </p>
            <p>
              <span className="text-muted-foreground">source</span> {structuredError.source}
            </p>
            <p>
              <span className="text-muted-foreground">message</span> {structuredError.message}
            </p>
            <p>
              <span className="text-muted-foreground">remediation</span>{" "}
              {structuredError.remediation}
            </p>
          </CardContent>
        </Card>
      )}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Persisted local execution log for recent function actions.
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Checklist gate, canary health, rollback playbook, drift summary, and verification
            matrix.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3 text-sm">
          <div className="space-y-1">
            ...
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={releaseChecklist.docs}
                onChange={(e) => setReleaseChecklist((v) => ({ ...v, docs: e.target.checked }))}
              />{" "}
              docs updated
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={releaseChecklist.tests}
                onChange={(e) => setReleaseChecklist((v) => ({ ...v, tests: e.target.checked }))}
              />{" "}
              tests passing
            </label>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={releaseChecklist.approvals}
                onChange={(e) =>
                  setReleaseChecklist((v) => ({ ...v, approvals: e.target.checked }))
                }
              />{" "}
              approvals complete
            </label>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Button size="sm" variant="outline" onClick={checkCanaryHealth}>
              Check Canary Health
            </Button>
            <span
              className={`rounded-full border px-2 py-1 text-xs ${canaryHealthy ? "border-green-500 text-green-600" : "border-red-500 text-red-600"}`}
            >
              canary {canaryHealthy ? "healthy" : "unhealthy"}
            </span>
            <span
              className={`rounded-full border px-2 py-1 text-xs ${releaseReady ? "border-green-500 text-green-600" : "border-yellow-500 text-yellow-600"}`}
            >
          ...
          <div className="space-y-1">
            <Button size="sm" onClick={runRollbackPlaybook}>
              Run Rollback Playbook Action
            </Button>
            {rollbackLog.length > 0 && (
              <ol className="list-decimal pl-5 text-xs space-y-1">
                {rollbackLog.map((step) => (
                  <li key={step}>{step}</li>
                ))}
              </ol>
            )}
          ...
          <div className="space-y-1">
            ...
            <div className="grid gap-1 text-xs">
              <div className="rounded border p-2">
                API health endpoint responds 200: {capabilities.health ? "yes" : "no"}
              </div>
              <div className="rounded border p-2">
                Workbench endpoints available: {capabilities.workbench ? "yes" : "no"}
              </div>
              <div className="rounded border p-2">
                Release gate ready: {releaseReady ? "yes" : "no"}
              </div>
            </div>
          </div>
        </CardContent>

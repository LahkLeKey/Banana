...
export function DataSciencePage() {
  ...
  const filteredOperations = useMemo(() => {
    ...
    return endpointOperations.filter((op) =>
  ...
  function loadEndpointBootstrapWorkspace() {
    ...
    const rebuiltCells: NotebookCell[] = bootstrapCells.map((cell, idx) => ({
      ...
      kind: cell.kind,
      source: cell.source,
      previewMode: cell.kind === "markdown",
    }));
  ...
  function injectEndpointCell(op: EndpointOperation) {
    ...
    const lines = [
      ...
      hasBody
        ? `payload = {}\nresult = call_endpoint('${op.method}', '${op.path}', payload)`
...
`);
  ...
  async function loadNotebookFromLibrary(entry: NotebookLibraryEntry) {
    ...
    try {
      ...
      setRuntimeMessage(
        `Failed to load ${entry.label}: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
                ...
                {cells.length === 0 ? (
                  ...
                  cells.map((cell, index) => (
                    <CellEditor
                      ...
                      onTogglePreview={() =>
                        updateCell(cell.id, { previewMode: !(cell.previewMode ?? true) })
                      }
                      onMoveUp={index > 0 ? () => moveCell(cell.id, "up") : undefined}
                      onMoveDown={
                        index < cells.length - 1 ? () => moveCell(cell.id, "down") : undefined
                      }

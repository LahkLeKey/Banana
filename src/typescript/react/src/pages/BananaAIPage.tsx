  ...
  async function run() {
    ...
    try {
      ...
      const lines = [
        ...
        `status: ${v.status}`,
        result.embedding
          ? `embedding: [${result.embedding.map((x) => x.toFixed(3)).join(", ")}]`
          : null,
      ]
        .filter(Boolean)
        .join("\n");
      ...
      const attributions = [
        ...
        {
          feature: "calibration_magnitude",
          weight: Number(((v.calibration_magnitude ?? 0) * 0.35).toFixed(3)),
        },
      ...
      const laneContributions = [
        ...
        {
          lane: "ripeness-model",
          pct: Math.round((0.2 + (v.calibration_magnitude ?? 0) * 0.1) * 100),
        },
      ];
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Unified assistant that composes Banana custom models into a single Rovo-style
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Routes your query through the ensemble model (Banana Classifier + Not-Banana Detector)
            for a combined verdict.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex gap-2">
            ...
            <Button
              variant="outline"
              onClick={runRipeness}
              disabled={ripenessLoading || !query.trim()}
            >
              ...
              {result.split("\n").map((line) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isEmbedding ? (
                      <span className="text-xs text-muted-foreground/60 truncate max-w-[60%]">
                        {val}
                      </span>
                    ) : (
          ...
          {ripenessError && <p className="text-sm text-destructive">{ripenessError}</p>}
          ...
          {explainability && (
            <div className="space-y-3 rounded-md border p-3">
                  ...
                  <div key={lane.lane} className="space-y-1">
                    ...
                    <div className="h-2 rounded bg-muted">
                      <div
                        ...
                        style={{ width: `${Math.max(0, Math.min(100, lane.pct))}%` }}
                      />
                    </div>
                  </div>
                ))}
              ...
              {explainability.oodReasons.length > 0 && (
                <div className="rounded border border-yellow-500/40 bg-yellow-500/10 p-2 text-sm">
                  ...
                  <ul className="list-disc pl-5 text-muted-foreground">
                    {explainability.oodReasons.map((reason) => (
              ...
              <Button
                variant="outline"
                onClick={() => navigator.clipboard.writeText(explainability.verdictJson)}
              >

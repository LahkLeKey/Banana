...
import { getKernelVariant, getWasmModule, WASM_INIT_TIMEOUT_MS } from "../lib/wasmRuntime";
...
const WASM_EXECUTION_TIMEOUT_CODE = 1001;
...
function dispatch(mod: BananaWasmModule, fn: WasmFn, payload: unknown): WasmResult<unknown> {
  switch (fn) {
...
self.onmessage = async (evt: MessageEvent<WorkerRequest>) => {
  ...
  const modOrTimeout = await Promise.race([
    ...
    new Promise<"timeout">((resolve) => {
      initTimeoutId = setTimeout(() => resolve("timeout"), WASM_INIT_TIMEOUT_MS);
    }),
  ]).finally(() => {
    if (initTimeoutId) {
  ...
  try {
    ...
    if (execDurationMs > CALL_TIMEOUT_MS) {
      emit({
        type: "error",
        id,
        ok: false,
        code: WASM_EXECUTION_TIMEOUT_CODE,

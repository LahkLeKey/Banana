 ...
 *   const worker = new Worker(new URL('./workers/bananaWasmWorker.ts', import.meta.url), { type: 'module' });
 *
 * Message protocol:
...
async function ensureModule(id: string): Promise<BananaWasmModule | null> {
  if (_permanentFallback) return null;
  if (_mod) return _mod;
...
function dispatch(
  ...
  switch (fn) {
    case 'calculate_banana': {
      const { purchases, multiplier } = payload as { purchases: number; multiplier: number };
    ...
    case 'binary': {
      const { inputJson } = payload as { inputJson: string };
      return classifyBanaryBinary(mod, inputJson);
    }
    case 'ripeness': {
      const { inputJson } = payload as { inputJson: string };
...
self.onmessage = async (evt: MessageEvent<WorkerRequest>) => {
  ...
  const modOrTimeout = await Promise.race([
    ensureModule(id),
    new Promise<'timeout'>((resolve) => setTimeout(() => resolve('timeout'), WASM_INIT_TIMEOUT_MS)),
  ]);
  ...
  try {
    ...
    if (result.ok) {
      ...
      telemetry('wasm.worker.call.error', { id, fn, code: result.code, message: result.message, channel: 'web' });
    }
  } catch (err) {
    clearTimeout(callTimeoutId);
    if (_cancelled.has(id)) { _cancelled.delete(id); return; }

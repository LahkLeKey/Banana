 ...
 * Used by GameEnginePage and cross-platform consumers (React, React Native, Electron)
 */

export type EngineStatus = "idle"|"loading"|"running"|"error"|"unavailable";

export type ContextMenuState = {
    visible: boolean; x : number; y : number; normalizedX : number; normalizedY : number;
};

export type ContextMenuAction = {
    label: string; actionId : string;
};

export type RadialDirection = "up"|"down"|"left"|"right"|null;

export type RadialControlState = {
    visible: boolean; x : number; y : number; activeDirection : RadialDirection;
};
...
export function computeContextMenuPosition(clientX: number, clientY: number, viewportWidth: number,
                                           viewportHeight: number, menuWidth = 220,
                                           menuHeight = 140, padding = 8): {x: number; y : number}
{
...
export function getDirectionFromTouch(touchX: number, touchY: number, radialCenterX: number,

...
import { RETRY_BUTTON_COPY, RetryButton } from "./RetryButton";
...
describe("RetryButton", () => {
  test("renders default copy", () => {
    act(() => {
      root.render(<RetryButton onClick={() => {}} />);
    });
  ...
  test("honors label override", () => {
    act(() => {
      root.render(<RetryButton onClick={() => {}} label="Retry capture" />);
    });
    const btn = container.querySelector('[data-testid="retry-button"]') as HTMLButtonElement;
  ...
  test("fires onClick when pressed", () => {
    const onClick = mock(() => {});
    act(() => {
      root.render(<RetryButton onClick={onClick} />);
    });
    const btn = container.querySelector('[data-testid="retry-button"]') as HTMLButtonElement;
    act(() => {
      btn.click();
  ...
  test("respects disabled prop", () => {
    const onClick = mock(() => {});
    act(() => {
      root.render(<RetryButton onClick={onClick} disabled />);
    });
    const btn = container.querySelector('[data-testid="retry-button"]') as HTMLButtonElement;

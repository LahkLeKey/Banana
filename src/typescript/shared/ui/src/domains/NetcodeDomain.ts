import {InputAggregator} from "./InputDomain";
import {InteractionManager} from "./InteractionDomain";

export type AntiCheatViolationReason =
    "sequence_regression"|"sequence_gap"|"tick_ahead"|"tick_behind"|"clock_skew"|"invalid_axis";
...
export class NetcodeDomain
{
    ...
    constructor(config: NetcodeDomainConfig, inputAggregator: InputAggregator,
    ...
    public ingestClientCommand(command: InputCommand, serverTick: number,
    ...
    private validateClientCommand(command: InputCommand, serverTick: number,
    ...
    private updateIntegrityHash(command: InputCommand): void
    {
        ...
        const inputMix =
            command.inputSequence * 31 + command.tickIndex * 17 + axisMix + timestampMix;

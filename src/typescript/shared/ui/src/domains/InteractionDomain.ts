 ...
 */
export type ActionHandler = (actionId: string, normalizedX: number, normalizedY: number) =>
    InteractionOutcome;
...
export class InteractionManager
{
     ...
     */
    openMenu(clientX: number, clientY: number, viewportWidth: number, viewportHeight: number,
             menuWidth = 220, menuHeight = 140): ContextMenuState
    {
        const padding = 8;
    ...
    executeAction(actionId: string): InteractionOutcome
    {
        ...
        {
            const outcome =
                handler(actionId, this.menuState.normalizedX, this.menuState.normalizedY);
            this.closeMenu();
            return outcome;
        }
        catch (error)
 ...
 */
export function createOutcome(actionId: string, normalizedX: number, normalizedY: number,
                              success: boolean, message: string): InteractionOutcome
{
    return {
        actionId,

...
export class ConnectionManager
{
    ...
    private heartbeatStats = new Map < HeartbeatLayerName,
    {
        sent: number;
        failed: number;
    ...
    checkHeartbeatHealth(nowMs = Date.now()): HeartbeatHealthReport
    {
        ...
        for (const layer of layers)
        {
            ...
            if (isAwaitingAck && nowMs - stats.lastSentAt > timeoutMs)
    ...
    private startHeartbeats(): void
    {
        ...
        {
            const timer =
                setInterval(() => { this.sendHeartbeat(layer.layer, layer.payloadFactory?.()); },
                            layer.intervalMs);
    ...
    private ensureLayerStats(layer: HeartbeatLayerName): {
        sent: number; failed : number; lastSentAt : number | null; lastAckAt : number | null;
        sequence : number;
    }
    {

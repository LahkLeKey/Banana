...
describe('createRequestQueue', () => {
  test('happy path drains and clears the job', async () => {
    const queue =
        createRequestQueue<string, string>({adapter: createInMemoryAdapter()});
    ...
    expect(report.outcomes).toEqual([
      {key: 'verdict:1', status: 'ok', result: 'got:hello'}
    ]);
  ...
  test('retry exhaustion drops the job after maxAttempts', async () => {
    ...
    expect(report.outcomes[0])
        .toEqual({key: 'verdict:1', status: 'retry', nextAttempts: 1});
    // Attempt 2
    report = await queue.drain(failing);
    expect(report.outcomes[0])
        .toEqual({key: 'verdict:1', status: 'retry', nextAttempts: 2});

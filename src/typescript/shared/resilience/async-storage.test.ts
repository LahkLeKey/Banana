...
import {createAsyncStorageAdapter, type AsyncStorageLike} from './src/adapters/async-storage';
...
describe('createAsyncStorageAdapter', () => {
  test('round-trips values and lists keys by prefix', async () => {
    ...
    expect(keys.sort()).toEqual(['banana:queue:default:a', 'banana:queue:default:b']);
    await adapter.delete('banana:queue:default:a');
    expect(await adapter.get('banana:queue:default:a')).toBeNull();
  });

  test('powers the same RequestQueue contract as IndexedDB adapter', async () => {
    const adapter = createAsyncStorageAdapter(makeMemoryAsyncStorage());
    const queue = createRequestQueue<string, string>({adapter});
    await queue.enqueue({key: 'verdict:1', payload: 'hello', retry: FAST_RETRY, enqueuedAt: 1});
    await queue.enqueue({key: 'verdict:1', payload: 'newer', retry: FAST_RETRY, enqueuedAt: 2});
    const handler = mock(async (p: string) => `got:${p}`);
    const report = await queue.drain(handler);
    expect(report.outcomes).toEqual([{key: 'verdict:1', status: 'ok', result: 'got:newer'}]);
  ...
  test('powers VerdictHistory ordering + cap', async () => {
    ...
    const v = (id: string, t: number): StoredVerdict => ({id, capturedAt: t, input: id, verdict: {id}, didEscalate: false});

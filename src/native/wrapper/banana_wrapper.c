...
int banana_calculate_banana_with_breakdown(int purchases, int multiplier, int *out_banana,
                                           int *out_base, int *out_bonus)
{
...
int banana_create_banana_message(int purchases, int multiplier, char **out_message)
{
    ...
    int written =
        snprintf(buf, 96, "purchases=%d multiplier=%d banana=%d", purchases, multiplier, total);
...
int banana_classify_banana_binary_with_threshold(const char *input_json, double threshold,
                                                 char **out_json)
{
...
int banana_classify_banana_transformer_ex(const char *input_json, int log_attention,
                                          double *out_embedding, double *out_attention_weights,
                                          char **out_json)
{
    return banana_wrapper_ml_classify_transformer_ex(input_json, log_attention, out_embedding,
                                                     out_attention_weights, out_json);
}

int banana_classify_banana_transformer_quant_embedding(const char *input_json,
                                                       signed char *out_quant, double *out_scale,
                                                       signed char *out_zero, char **out_json)
{
    return banana_wrapper_ml_classify_transformer_quant_embedding(input_json, out_quant, out_scale,
                                                                  out_zero, out_json);
}
...
int banana_add_bunch_to_harvest_batch(const char *batch_id, const char *input_json, char **out_json)
{
...
int banana_load_truck_container(const char *truck_id, const char *input_json, char **out_json)
{
    return banana_core_load_truck_container(truck_id, input_json, out_json);
}

int banana_unload_truck_container(const char *truck_id, const char *container_id, char **out_json)
{
    return banana_core_unload_truck_container(truck_id, container_id, out_json);
}

int banana_relocate_truck(const char *truck_id, const char *input_json, char **out_json)
{
...
int banana_anticheat_reset_session(const char *session_id)
{

                                     ...
                                     const char *model_name, char **out_json)
{
    ...
    required = snprintf(NULL, 0,
                        ...
                        model_name, result->label, result->confidence, result->banana_score,
                        result->not_banana_score, result->jaccard, result->confusion.tp,
                        result->confusion.fp, result->confusion.fn, result->confusion.tn);
    ...
    if (snprintf(payload, (size_t)required + 1u,
                 ...
                 model_name, result->label, result->confidence, result->banana_score,
                 result->not_banana_score, result->jaccard, result->confusion.tp,
                 result->confusion.fp, result->confusion.fn, result->confusion.tn) < 0)
    {
        free(payload);
        return BANANA_INTERNAL_ERROR;
    }

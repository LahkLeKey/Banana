...
void camera_look_at(Camera *c, float px, float py, float pz, float tx, float ty, float tz)
{
    c->position[0] = px;
    c->position[1] = py;
    c->position[2] = pz;
...
{
    float f[3] = {c->target[0] - c->position[0], c->target[1] - c->position[1],
    ...
    m16[4] = s[1];
    m16[8] = s[2];
    m16[12] = -dot3(s, c->position);
    m16[1] = u[0];
    m16[5] = u[1];
    m16[9] = u[2];
    m16[13] = -dot3(u, c->position);
    m16[2] = -f[0];
    m16[6] = -f[1];
    m16[10] = -f[2];
    m16[14] = dot3(f, c->position);
    m16[15] = 1.f;
}

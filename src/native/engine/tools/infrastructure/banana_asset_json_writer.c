...
int banana_write_palette_json(const char *path) {
  ...
  for (int i = 0; i < banana_tile_count(); i++) {
    fprintf(
        f,
        "    {\"id\":\"%s\",\"biome\":%d,\"elevationBase\":%d,\"resourceBias\":%d}%s\n",
        defs[i].id,
        defs[i].biome,
        defs[i].elevation_base,
        defs[i].resource_bias,
...
int banana_write_terrain_json(
  ...
  for (int i = 0; i < cell_count; i++) {
    ...
    fprintf(
        f,
        "    {\"x\":%d,\"y\":%d,\"tile\":\"%s\",\"elevation\":%d,\"resource\":%d}%s\n",
        x,
        y,
        defs[cells[i].tile_index].id,
        cells[i].elevation,
        cells[i].resource,
        (i + 1 < cell_count) ? "," : "");
  }
...
int banana_write_manifest_json(
  ...
  fprintf(
      ...
      "    {\"name\":\"terrain\",\"file\":\"banana-generated-terrain.json\",\"checksum\":\"0x%016llX\"}\n"
      "  ]\n"
      "}\n",
      config->profile,
      config->seed,

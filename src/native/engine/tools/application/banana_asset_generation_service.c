...
int banana_generate_terrain_cells(
  ...
  return banana_generate_terrain_cells_with_contract(
      config,
      cellular_iterations,
...
int banana_generate_terrain_cells_with_contract(
    ...
    banana_terrain_cell_t *out_cells,
    uint64_t *out_checksum) {
  if (!config || !contract || !contract->generate_tiles || !contract->post_process_tiles || !out_cells || !out_checksum) {
    return 0;
  }

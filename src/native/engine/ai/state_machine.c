...
int fsm_add_state(StateMachine *fsm, const char *name, void (*on_enter)(void *),
                  void (*on_update)(void *, float), void (*on_exit)(void *))
{
...
{
    if (fsm->transition_count >= BANANA_FSM_MAX_TRANSITIONS)
        return;
...
void fsm_trigger(StateMachine *fsm, const char *trigger)
{
    for (int i = 0; i < fsm->transition_count; i++)
    {
            ...
            strncmp(t->trigger, trigger, BANANA_FSM_NAME_LEN) == 0)
        {
            ...
            int to = t->to_state;
            if (fsm->states[from].on_exit)
                fsm->states[from].on_exit(fsm->context);
            fsm->current_state = to;
            fsm->time_in_state = 0.f;
            if (fsm->states[to].on_enter)
                fsm->states[to].on_enter(fsm->context);
            return;
        }
    }
}

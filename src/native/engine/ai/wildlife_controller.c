...
static void idle_exit(void *ctx) { (void)ctx; }

/* ── patrol ── */
static void patrol_enter(void *ctx) {
    WildlifeState *w = (WildlifeState *)ctx;
    float *wp = w->patrol_waypoints[w->patrol_wp_idx];
    nav_find_path(&w->grid, w->position[0], w->position[2],
                  wp[0], wp[2], &w->current_path);
}
...
static void patrol_exit(void *ctx) { (void)ctx; }
...
static void investigate_enter(void *ctx) {
    ...
    nav_find_path(&w->grid, w->position[0], w->position[2],
                  w->investigate_target[0], w->investigate_target[2],
                  &w->current_path);
}
...
static void investigate_exit(void *ctx) { (void)ctx; }

/* ── return ── */
static void return_enter(void *ctx) {
    WildlifeState *w = (WildlifeState *)ctx;
    nav_find_path(&w->grid, w->position[0], w->position[2],
                  w->patrol_origin[0], w->patrol_origin[2], &w->current_path);
}
...
static void return_exit(void *ctx) { (void)ctx; }
...
static void wildlife_signal(ControllerInstance *self,
...
static ControllerInstance *wildlife_factory(float x, float y, float z) {
    ...
    w->position[0] = x; w->position[1] = y; w->position[2] = z;
    w->patrol_origin[0] = x; w->patrol_origin[1] = y; w->patrol_origin[2] = z;
    ...
    int inv_idx   = fsm_add_state(&w->fsm, "investigate", investigate_enter, investigate_update, investigate_exit);

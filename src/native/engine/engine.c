...
static Material mat_for_entity(EntityType t) {
    switch (t) {
    case ENTITY_TYPE_NPC:     return material_solid(0.20f, 0.80f, 0.30f, 1.f); /* green  */
    case ENTITY_TYPE_DYNAMIC: return material_solid(0.30f, 0.55f, 0.90f, 1.f); /* blue   */
    case ENTITY_TYPE_PLAYER:  return material_solid(0.95f, 0.75f, 0.10f, 1.f); /* yellow */
    default:                  return material_solid(0.55f, 0.55f, 0.55f, 1.f); /* gray   */
    }
}
...
int engine_tick(float dt) {
    ...
    if (s_cube && s_world) {
        ...
        for (int j = 0; j < s_world->entity_count; j++) {
            ...
            renderer_draw_mesh(s_renderer, s_cube,
                               e->position, identity, scale, &mat);
        }
    }
...
uint32_t engine_controller_create(const char *type, float x, float y, float z) {
    if (s_controller_count >= BANANA_MAX_ACTIVE_CONTROLLERS) return 0;
...
uint32_t engine_controller_attach(uint32_t entity_id, const char *type) {
    ...
    if (!e) return 0;
    uint32_t cid = engine_controller_create(type, e->position[0],
...
float engine_get_entity_x(int idx) {
    if (!s_world || idx < 0 || idx >= s_world->entity_count) return 0.f;
...
int engine_get_entity_state(int idx) {
    if (!s_world || idx < 0 || idx >= s_world->entity_count) return 0;
    Entity *e = s_world->entities[idx];
    if (!e || e->controller_id == 0) return 0;
    for (int i = 0; i < s_controller_count; i++) {
        ControllerInstance *c = s_controllers[i];
...
static void _mat4_force_link(void) { (void)_mat4_mul_ref; }

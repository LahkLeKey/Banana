...
- `runtime/engine/lifecycle/engine_lifecycle*.*`: bootstrap/teardown orchestration.
- `runtime/engine/tick/engine_tick.*`: tick pipeline execution and phase ordering.
...
Native validation now uses a human-feedback loop factory under
`tests/native/feedback/native_feedback_loop_factory.c`.

- CTest smoke targets:
  - `banana_native_feedback_loop_factory_smoke` for gameplay script coverage.
  - `banana_native_feedback_loop_factory_dx12_scaffold_smoke` for scaffold lifecycle coverage.
- Scenario loops (`warfront`, `negotiate`, `comeback`) and Playwright-style DX12
  playloop scripts (`*.dx12play`) are run via `scripts/run-native-feedback-loop.sh`
  and `scripts/run-war-test-suites.sh`.
- DX12 scaffold scripts can bootstrap client lifecycle coverage with
  `test.use dx12-client-scaffold`, plus aliases
  `expect.toHaveMetric`, `expect.toHaveMode`, and `expect.toHaveTeamCount`.
- Evidence capture for spec 031 is generated by
  `scripts/refresh-coherent-world-evidence.sh`.

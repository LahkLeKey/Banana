...
Current delivery focus is a stable native runtime with clear module boundaries.
...
```
Desktop shell (native runtime host)
...
## Native render runtime contract (current)

- Default backend is `glfw` (see `BANANA_RENDER_BACKEND` in CMake).
...
- Scenario loops (`warfront`, `negotiate`, `comeback`) and playloop scripts
  (`*.dx12play`) are run via `scripts/run-native-feedback-loop.sh`

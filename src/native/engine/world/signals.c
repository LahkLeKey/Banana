...
SignalQueue *signals_get_queue(void) {
    if (!s_initialized) { memset(&s_queue, 0, sizeof(s_queue)); s_initialized = 1; }
    return &s_queue;
}
...
void signals_send(uint32_t target_id, const char *name, const char *data) {
    ...
    if (data) strncpy(s->data, data, BANANA_SIGNAL_DATA_LEN - 1);
    else s->data[0] = '\0';
    q->count++;
}
...
void signals_flush(struct World *world) {
    ...
    while (q->count > 0) {
        ...
        if (e && e->controller_id) {
            /* Route to controller — controller registry lookup in T1c */

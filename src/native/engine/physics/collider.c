...
int collider_box_vs_box(const PhysicsBody *a, const PhysicsBody *b, Collision *out) {
    ...
    if (ox < oy && ox < oz) {
        out->penetration = ox;
        out->normal[0] = dx < 0 ? -1.f : 1.f; out->normal[1] = 0; out->normal[2] = 0;
    } else if (oy < oz) {
        out->penetration = oy;
        out->normal[0] = 0; out->normal[1] = dy < 0 ? -1.f : 1.f; out->normal[2] = 0;
    } else {
        out->penetration = oz;
        out->normal[0] = 0; out->normal[1] = 0; out->normal[2] = dz < 0 ? -1.f : 1.f;
    }
    return 1;
}
...
int collider_sphere_vs_sphere(const PhysicsBody *a, const PhysicsBody *b, Collision *out) {
    ...
    if (dist2 >= radsum * radsum) return 0;
    ...
    if (dist > 1e-6f) {
        out->normal[0] = dx/dist; out->normal[1] = dy/dist; out->normal[2] = dz/dist;
    } else {
...
CollisionList collider_broad_phase(PhysicsBody **bodies, int count) {
    CollisionList list; list.count = 0;
    for (int i = 0; i < count && list.count < BANANA_MAX_COLLISIONS; i++) {
        for (int j = i+1; j < count && list.count < BANANA_MAX_COLLISIONS; j++) {
            ...
            if (hit) list.items[list.count++] = c;
        }
    }
    return list;
}
...
void collider_resolve(const Collision *c, PhysicsBody *a, PhysicsBody *b) {
    ...
    if (!a->is_static) { a->velocity[0] -= jx*inv_mass_a; a->velocity[1] -= jy*inv_mass_a; a->velocity[2] -= jz*inv_mass_a; }
    if (!b->is_static) { b->velocity[0] += jx*inv_mass_b; b->velocity[1] += jy*inv_mass_b; b->velocity[2] += jz*inv_mass_b; }

    /* Positional correction to prevent sinking */
    float correction = (c->penetration / inv_sum) * 0.8f;
    if (!a->is_static) { a->position[0] -= c->normal[0]*correction*inv_mass_a; a->position[1] -= c->normal[1]*correction*inv_mass_a; a->position[2] -= c->normal[2]*correction*inv_mass_a; }
    if (!b->is_static) { b->position[0] += c->normal[0]*correction*inv_mass_b; b->position[1] += c->normal[1]*correction*inv_mass_b; b->position[2] += c->normal[2]*correction*inv_mass_b; }
}

...
extern "C"
{
    ...
    int banana_core_load_truck_container(const char *truck_id, const char *input_json,
                                         char **out_json);
    ...
    /* Anti-cheat domain (server-authoritative, usermode telemetry ingestion). */
    int banana_core_anticheat_reset_session(const char *session_id);
    int banana_core_anticheat_submit_usermode_heartbeat(

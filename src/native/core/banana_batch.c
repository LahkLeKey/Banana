...
static const char *infer_ripeness_hint(const char *text)
{
    ...
    int ripe =
        count_term_ci(text, "ripe") + count_term_ci(text, "yellow") + count_term_ci(text, "sweet");
    int overripe = count_term_ci(text, "overripe") + count_term_ci(text, "brown") +
                   count_term_ci(text, "mushy");

    if (unripe >= ripe && unripe >= overripe)
        return "unripe";
    if (overripe > ripe)
        return "overripe";
    return "ripe";
...
static HarvestBatchRecord *find_harvest_batch(const char *id)
{
    ...
    {
        if (strcmp(g_harvest_batches[i].id, id) == 0)
            return &g_harvest_batches[i];
    }
    return NULL;
}
...
int banana_core_create_batch(const char *input_json, char **out_json)
{
    ...
    written = snprintf(payload, sizeof(payload),
                       "{\"batch_id\":\"%s\",\"status\":\"created\",\"item_count_estimate\":%d,"
                       "\"ripeness_hint\":\"%s\"}",
...
int banana_core_get_batch_status(const char *batch_id, char **out_json)
{
    ...
    written = snprintf(payload, sizeof(payload),
                       "{\"batch_id\":\"%s\",\"status\":\"active\",\"item_count_estimate\":%d,"
                       "\"ripeness_hint\":\"%s\"}",
                       record->id, record->item_count_estimate, record->ripeness_hint);
    if (written < 0 || written >= (int)sizeof(payload))
        return BANANA_OVERFLOW;
...
int banana_core_predict_batch_ripeness(const char *batch_id, char **out_json)
{
    ...
    confidence = 0.60;
    if (strcmp(record->ripeness_hint, "ripe") == 0)
        confidence = 0.82;
    else if (strcmp(record->ripeness_hint, "overripe") == 0)
        confidence = 0.78;
    else if (strcmp(record->ripeness_hint, "unripe") == 0)
        confidence = 0.74;

    written = snprintf(payload, sizeof(payload),
                       "{\"batch_id\":\"%s\",\"label\":\"%s\",\"confidence\":%.6f}", record->id,
                       record->ripeness_hint, confidence);
    if (written < 0 || written >= (int)sizeof(payload))
        return BANANA_OVERFLOW;
                                           ...
                                           char **out_json)
{
    ...
    if (!batch_id || !input_json || !out_json)
        return BANANA_INVALID_ARGUMENT;
    ...
    if (bunch_increment < 1)
        bunch_increment = 1;
    if (bunch_increment > 8)
        bunch_increment = 8;

...
static const BananaMlBigramPattern k_negative_bigrams[] = {
    {"engine", "oil"},
    {"plastic", "waste"},
    {"chemical", "spill"},
    {"junk", "data"},
...
static int banana_ml_match_bigram(const char* prev,
                                  const char* current,
                                  const BananaMlBigramPattern* patterns,
                                  size_t count) {
    size_t i;
    for (i = 0; i < count; ++i) {
...
static int banana_ml_extract_key_text(const char* input_json,
                                      const char* key,
                                      char* out_text,
...
static int banana_ml_load_candidate_text(const char* input_json,
                                         char* candidate,
...
void banana_ml_fill_confusion(int predicted_is_banana,
...
int banana_ml_build_feature_vector(const char* input_json, BananaMlFeatureVector* out_features) {
    ...
    while (normalized[i] != '\0') {
        ...
        banana_hit = banana_ml_token_in_list(token,
                                             k_banana_signal_tokens,
                                             k_banana_signal_tokens_count);
        not_banana_hit = banana_ml_token_in_list(token,
                                                 k_not_banana_signal_tokens,
                                                 sizeof(k_not_banana_signal_tokens) / sizeof(k_not_banana_signal_tokens[0]));
        ...
        if (prev_token[0] != '\0') {
            if (banana_ml_match_bigram(prev_token,
                                       token,
                                       k_positive_bigrams,
    ...
    out_features->banana_signal_ratio = (double)out_features->banana_hits / (double)out_features->token_count;
    out_features->not_banana_signal_ratio = (double)out_features->not_banana_hits / (double)out_features->token_count;
    out_features->unique_token_ratio = (double)out_features->unique_token_count / (double)out_features->token_count;
    ...
    if (out_features->token_count > 1) {
        ...
        out_features->positive_bigram_ratio = (double)out_features->positive_bigram_hits / bigram_denominator;
        out_features->negative_bigram_ratio = (double)out_features->negative_bigram_hits / bigram_denominator;
    } else {
    ...
    out_features->not_banana_signal_ratio = banana_ml_clamp01(out_features->not_banana_signal_ratio);
    ...
    out_features->not_banana_attention_ratio = banana_ml_clamp01(out_features->not_banana_attention_ratio);

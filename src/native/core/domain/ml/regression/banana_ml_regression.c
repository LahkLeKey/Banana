...
int banana_ml_regression_predict(const BananaMlFeatureVector *features, double *out_score)
{
    ...
    linear = 0.20 + (1.40 * features->banana_signal_ratio) -
             (1.10 * features->not_banana_signal_ratio) + (0.30 * features->unique_token_ratio) +
             (0.20 * features->length_ratio) + (0.40 * features->positive_bigram_ratio) -
             (0.30 * features->negative_bigram_ratio) + (0.35 * features->banana_attention_ratio) -

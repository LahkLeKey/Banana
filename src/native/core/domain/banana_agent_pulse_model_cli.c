...
int main(void)
{
    ...
    {
        printf("    "
               "{\"agent\":\"%s\",\"priority\":%d,\"deterministic_snapshots\":%d,\"focus\":\"%s\"}",
               lanes[index].agent, lanes[index].deterministic_priority,
               lanes[index].deterministic_snapshots, lanes[index].focus);

...
static TruckRecord *find_truck(const char *truck_id)
{
    ...
    {
        if (strcmp(g_trucks[i].id, truck_id) == 0)
            return &g_trucks[i];
    }
    return NULL;
}
...
static const char *infer_location(const char *text)
{
    if (!text)
        return "yard";
    if (contains_ci(text, "warehouse"))
        return "warehouse";
    if (contains_ci(text, "port"))
        return "port";
    if (contains_ci(text, "farm"))
        return "farm";
    if (contains_ci(text, "market"))
        return "market";
    return "yard";
}
...
int banana_core_register_truck(const char *input_json, char **out_json)
{
    ...
    if (!input_json || !out_json)
        return BANANA_INVALID_ARGUMENT;
    if (g_truck_count >= BANANA_MAX_TRUCKS)
        return BANANA_INTERNAL_ERROR;
    ...
    written = snprintf(payload, sizeof(payload),
                       "{\"truck_id\":\"%s\",\"status\":\"registered\",\"location\":\"%s\","
                       "\"container_count\":%d}",
                       record->id, record->location, record->container_count);
...
int banana_core_load_truck_container(const char *truck_id, const char *input_json, char **out_json)
{
    ...
    if (!truck_id || !input_json || !out_json)
        return BANANA_INVALID_ARGUMENT;
                                       ...
                                       char **out_json)
{
    ...
    if (!truck_id || !container_id || !out_json)
        return BANANA_INVALID_ARGUMENT;

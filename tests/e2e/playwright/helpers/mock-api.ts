...
export async function routeEnsembleSuccess(page: Page): Promise<void> {
  await page.route("**/api/ensemble**", (route: Route) =>
    route.fulfill({
      status: 200,
...
export const MOCK_CHAT_SESSION = { session_id: "mock-session-001", ready: true };
...
export async function routeChatReady(page: Page): Promise<void> {
  await page.route("**/api/chat/session**", (route: Route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  await page.route("**/chat/session**", (route: Route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify(MOCK_CHAT_SESSION),
...
export async function routeChatMessageReply(page: Page): Promise<void> {
  await page.route("**/api/chat/message**", (route: Route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
  ...
  await page.route("**/chat/message**", (route: Route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify(MOCK_CHAT_MESSAGE),
...
export async function routeRipenessSuccess(page: Page): Promise<void> {
  await page.route("**/api/ripeness**", (route: Route) =>
    route.fulfill({
      status: 200,
      contentType: "application/json",
...
export async function routeAllBackgroundCalls(page: Page): Promise<void> {
  ...
  await page.route("**/api/summary**", (route: Route) =>

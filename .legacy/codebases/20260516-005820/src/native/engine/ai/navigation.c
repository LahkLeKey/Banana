...
{
    if (col < 0 || col >= NAV_GRID_W || row < 0 || row >= NAV_GRID_H)
        return;
...
/* Clamp to grid */
#define CLAMP_GRID(v, max)                                                                         \
    if (v < 0)                                                                                     \
        v = 0;                                                                                     \
    if (v >= max)                                                                                  \
    v = max - 1
    CLAMP_GRID(sc, NAV_GRID_W);
    CLAMP_GRID(sr, NAV_GRID_H);
    CLAMP_GRID(gc, NAV_GRID_W);
    CLAMP_GRID(gr, NAV_GRID_H);
    ...
    nodes[sc][sr].col = sc;
    nodes[sc][sr].row = sr;
    nodes[sc][sr].parent_col = -1;
    nodes[sc][sr].parent_row = -1;
    ...
    for (int iter = 0; iter < NAV_GRID_W * NAV_GRID_H * 4; iter++)
    {
        ...
        for (int d = 0; d < 8; d++)
        {
            ...
            if (nc < 0 || nc >= NAV_GRID_W || nr < 0 || nr >= NAV_GRID_H)
                continue;
            if (!g->passable[nc][nr] || nodes[nc][nr].closed)
                continue;
            int cost = d < 4 ? 10 : 14;
...
{
    if (path->current_index >= path->count)
        return 1;

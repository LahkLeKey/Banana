#include "engine.h"
#include "ai/ml_agent_controller.h"
#include "ai/player_controller.h"
#include "ai/wildlife_controller.h"
#include "domain/services/batch_engine_bridge.h"
#include "domain/services/cellular_worldgen_service.h"
#include "domain/services/ml_controller_bridge.h"
#include "domain/services/wfc_worldgen_service.h"
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define BANANA_TERRAIN_SIZE 48
#define BANANA_TERRAIN_MAX_LAYERS 7
#define BANANA_TERRAIN_CHUNK_SIZE 8
...
static int terrain_rebuild_chunk(int chunk_x, int chunk_z)
{
...

    {
        ...
        float max_layer = (float)(BANANA_TERRAIN_MAX_LAYERS - 1);
        ...
        for (int z = 0; z < BANANA_TERRAIN_CHUNK_SIZE; z++)
        {
            for (int x = 0; x < BANANA_TERRAIN_CHUNK_SIZE; x++)
            {
                ...
                if (gx >= BANANA_TERRAIN_SIZE)
                    gx = BANANA_TERRAIN_SIZE - 1;
                if (gz >= BANANA_TERRAIN_SIZE)
                    gz = BANANA_TERRAIN_SIZE - 1;
                if (gx_r >= BANANA_TERRAIN_SIZE)
                    gx_r = BANANA_TERRAIN_SIZE - 1;
                if (gz_d >= BANANA_TERRAIN_SIZE)
                    gz_d = BANANA_TERRAIN_SIZE - 1;

                h = (float)s_terrain_height[gz][gx];
                h_r = (float)s_terrain_height[gz][gx_r];
                h_d = (float)s_terrain_height[gz_d][gx];

                sum_height += h;
                slope_accum += fabsf(h - h_r) + fabsf(h - h_d);
                count++;
            }
        }

        if (count > 0 && max_layer > 0.0f)
        {
            int idx = terrain_chunk_index(chunk_x, chunk_z);
            s_terrain_chunks[idx].avg_height =
                clampf_local((sum_height / (float)count) / max_layer, 0.0f, 1.0f);
            s_terrain_chunks[idx].ruggedness =
                clampf_local(slope_accum / ((float)count * max_layer * 2.0f), 0.0f, 1.0f);
        }
    }
...
{
    engine_wfc_generate_heightfield(&s_terrain_height[0][0], BANANA_TERRAIN_SIZE,
                                     BANANA_TERRAIN_SIZE, BANANA_TERRAIN_MAX_LAYERS,
                                     20260509u);
    engine_cellular_smooth_heightfield(&s_terrain_height[0][0], BANANA_TERRAIN_SIZE,
                                       BANANA_TERRAIN_SIZE,
                                       BANANA_TERRAIN_MAX_LAYERS, 3);
...
static void terrain_draw(void)
{
    ...
    for (int cz = 0; cz < BANANA_TERRAIN_CHUNK_ROWS; cz++)
    {
        for (int cx = 0; cx < BANANA_TERRAIN_CHUNK_COLS; cx++)
        {
            ...
            h = s_terrain_chunks[idx].avg_height;
            r = s_terrain_chunks[idx].ruggedness;
            rr = lerpf_local(0.24f, 0.58f, h) * lerpf_local(1.00f, 0.88f, r);
            rg = lerpf_local(0.52f, 0.55f, h) * lerpf_local(1.00f, 0.84f, r);
            rb = lerpf_local(0.25f, 0.40f, h) * lerpf_local(1.00f, 0.80f, r);

            terrain_mat = material_solid(clampf_local(rr, 0.0f, 1.0f),
                                         clampf_local(rg, 0.0f, 1.0f),
                                         clampf_local(rb, 0.0f, 1.0f), 1.f);
            terrain_mat.use_texture = 1.0f;
            terrain_mat.uv_scale = lerpf_local(6.0f, 12.0f, r);
...
static void update_player_motion(float dt)
{
...

    {
        ...
        ControllerInstance *player_controller = NULL;

        if (player->controller_id)
            player_controller = engine_controller_find(player->controller_id);
        if (player_controller)
            player_controller_get_move_input(player_controller, &move_x, &move_z);
...

        {
            ...
            if (player_controller)
            {
                player_controller->position[0] = player->position[0];
                player_controller->position[1] = player->position[1];
                player_controller->position[2] = player->position[2];
            }
        }
    }
}
...
int engine_init(int width, int height)
{
    ...
    player_controller_register();
    ml_agent_controller_register();
    ...
    if (s_player_id)
    {
        ...
        if (player)
        {
            engine_controller_attach(s_player_id, "player");
    ...
    {
        const float actor_positions[8][2] = {
            {-12.0f, -10.5f},
            {10.5f, -12.0f},
            {-9.0f, 11.5f},
            {12.5f, 9.0f},
        ...
        const EntityType actor_types[8] = {
            ENTITY_TYPE_NPC,
            ENTITY_TYPE_DYNAMIC,
            ENTITY_TYPE_NPC,
            ENTITY_TYPE_DYNAMIC,
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (actor)
            {
                ...
                actor->scale[1] = 1.0f;
                actor->scale[2] = 1.0f;
                engine_controller_attach(actor_id, "ml-agent");
            }
        }
    }
...
int engine_tick(float dt)
{
    ...
    for (int i = 0; i < s_controller_count; i++)
    {
        ...
        if (strncmp(c->type_name, "ml", 2) == 0)
        {
            ...
            if (engine_ml_compute_move(c->id, c->position[0], c->position[1], c->position[2], dt,
                                       &move_x, &move_z) == 0)
            {
                c->position[0] += move_x * speed * dt;
                c->position[2] += move_z * speed * dt;
                c->position[0] = clampf_local(c->position[0], -island_span, island_span);
                c->position[2] = clampf_local(c->position[2], -island_span, island_span);
                c->position[1] = terrain_sample_height(c->position[0], c->position[2]) + 0.55f;
            }
        }
        else
        {
...
{
    ControllerInstance *controller = engine_controller_find(id);
    if (controller)
...
void engine_controller_signal(uint32_t id, const char *sig)
{
    ControllerInstance *controller = engine_controller_find(id);
    if (controller)
        controller_signal(controller, sig, NULL);
}

ControllerInstance *engine_controller_find(uint32_t id)
{
    for (int i = 0; i < s_controller_count; i++)
        if (s_controllers[i] && s_controllers[i]->id == id)
...
void engine_set_move_input(float input_x, float input_z)
{
    ...
    engine_ml_record_player_input(s_move_input_x, s_move_input_z);

    if (s_world && s_player_id)
    {
        Entity *player = world_get_entity(s_world, s_player_id);
        if (player && player->controller_id)
        {
            ControllerInstance *controller = engine_controller_find(player->controller_id);
            if (controller)
                player_controller_set_move_input(controller, s_move_input_x, s_move_input_z);
        }
    }
    ...
    {
        char payload[64];
        snprintf(payload, sizeof(payload), "%g,%g", s_move_input_x, s_move_input_z);
        signals_send(s_player_id, "move_input", payload);
    }
}

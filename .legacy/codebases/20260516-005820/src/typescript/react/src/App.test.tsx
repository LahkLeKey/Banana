...
describe("App ensemble verdict surface (slice 023)", () => {
  ...
  test("shows bootstrap remediation and recovers after retry", async () => {
    ...
    expect(screen.getByTestId("chat-bootstrap-remediation").textContent).toContain(
      "Ensure the runtime profile is healthy and the apps profile is up"
    );
  ...
  test("renders empty-state copy before any submit", async () => {
    ...
    expect(screen.getByTestId("ensemble-verdict-empty").textContent).toBe(
      "Send a sample to get a verdict."
    );
  });
  ...
  test("renders escalated banana copy + EscalationPanel trigger", async () => {
    ...
    expect(screen.getByTestId("ensemble-verdict-copy").textContent).toContain(
  ...
  test("shows retry button after error and reuses last submitted draft", async () => {
    ...
    expect(screen.getByTestId("ensemble-error").textContent).toBe(
      "Banana service is having a moment. Try again."
    );

import type {ChatMessage, ChatSession, EnsembleVerdict, RipenessResult,} from "@banana/ui";
import type {
    TrainingAuditEvent, TrainingLane, TrainingRunRequest, TrainingRunResult,} from
    "@banana/ui/training";
...
export type PromoteRunResponse = {
    promoted: {run_id: string; target : "candidate" | "stable"; threshold_passed : boolean};
...
export type ChatBootstrapErrorKind = "transport_unreachable"|"request_failed"|"unexpected";

export type ChatBootstrapErrorResolution = {
    kind: ChatBootstrapErrorKind; message : string; remediation : string; retryable : boolean;
};
...
{
    const importMeta = import.meta as {env?: {VITE_BANANA_CHAT_API_BASE_URL?: string}};
...
export function resolveApiBaseResolution(
    viteBaseUrl = resolveViteApiBaseUrl(), electronBridge = resolveElectronBridge(),
...
{
    return resolveApiBaseResolution(viteBaseUrl, electronBridge, localhostDefault).error;
}

export function resolveApiBaseUrl(viteBaseUrl = resolveViteApiBaseUrl(),
                                  electronBridge = resolveElectronBridge(),
                                  localhostDefault = resolveLocalhostDefaultApiBaseUrl()): string
{
    return resolveApiBaseResolution(viteBaseUrl, electronBridge, localhostDefault).baseUrl;
}

export function resolvePlatformLabel(electronBridge = resolveElectronBridge()): string
{
    return electronBridge?.platform ? `electron-${electronBridge.platform}` : "web";
}
...
function resolveLocalhostFastifyChatBase(baseUrl: string): string
{
    ...
    try
    {
        ...
        if ((host === "localhost" || host === "127.0.0.1") && parsed.port === "8080")
        {
            parsed.port = "8081";
...
export function resolveChatApiBaseUrl(apiBaseUrl: string,
                                      viteChatApiBaseUrl = resolveViteChatApiBaseUrl(),
...
export function resolveChatBootstrapError(error: unknown): ChatBootstrapErrorResolution
{
    ...
    return {
        ...
        remediation : "Check runtime logs and frontend diagnostics, then retry after remediation.",
        retryable : true,
    };
}
...
export async function sendChatMessage(
    baseUrl: string, sessionId: string, content: string, clientMessageId: string,
    chatApiBaseUrl = resolveChatApiBaseUrl(baseUrl)): Promise<SendMessageResponse>
{
    return requestJson<SendMessageResponse>(chatApiBaseUrl, `/chat/sessions/${sessionId}/messages`,
                                            {
                                                method : "POST",
                                                headers : {"content-type" : "application/json"},
                                                body : JSON.stringify({
 ...
 */
export async function fetchEnsembleVerdict(baseUrl: string, text: string): Promise<EnsembleVerdict>
{
...
export async function fetchEnsembleVerdictWithEmbedding(baseUrl: string, text: string):
...
export async function runTrainingWorkbench(baseUrl: string, payload: TrainingRunRequest):
...
export async function fetchTrainingWorkbenchHistory(baseUrl: string, lane?: TrainingLane):
    Promise<TrainingHistoryResponse>
{
    const suffix = lane ? `?lane=${encodeURIComponent(lane)}` : "";
    return requestJson<TrainingHistoryResponse>(baseUrl, `/training/workbench/history${suffix}`);
}

export async function fetchTrainingWorkbenchRun(baseUrl: string,
                                                runId: string): Promise<TrainingRunDetailResponse>
{
    return requestJson<TrainingRunDetailResponse>(
        baseUrl, `/training/workbench/runs/${encodeURIComponent(runId)}`);
}

export async function promoteTrainingWorkbenchRun(baseUrl: string, runId: string,
                                                  target: "candidate"|"stable", operator_id: string,
                                                  reason?: string): Promise<PromoteRunResponse>
{

...
const WASM_JSON_PARSE_ERROR_CODE = 3;

function parseJsonOrError<T>(jsonStr: string): WasmResult<T> {
  try {
    ...
    return {
      ok: false,
      code: WASM_JSON_PARSE_ERROR_CODE,
      message: `invalid_json_output: ${message}`,
    };
  }
}
...
export function calculateBanana(
  ...
  try {
    ...
    if (status !== 0) return { ok: false, code: status, message: wasmStatusMessage(status) };
...
export function classifyBananaBinary(
  ...
  try {
    ...
    return parseJsonOrError<BinaryClassifyResult>(jsonStr);
  } finally {
    mod._free(inputPtr);
    mod._free(outPtrPtr);
  }
...
export function classifyBananaBinaryWithThreshold(
  ...
  try {
    const status = mod._banana_classify_banana_binary_with_threshold(
      inputPtr,
      threshold,
      outPtrPtr
    );
...
export function predictRipeness(
  ...
  try {
    ...
    const jsonStr = mod.UTF8ToString(jsonPtr);
    mod._banana_free(jsonPtr);
    return parseJsonOrError<RipenessResult>(jsonStr);
  } finally {
    mod._free(inputPtr);
    mod._free(outPtrPtr);
  }

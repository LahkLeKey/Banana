...
function selectAssetSourceDir() {
  ...
  return { dir: generatedSourceDir, files };
}

function copyToPublic() {
  const selected = selectAssetSourceDir();
  if (!selected) {
  ...
  for (const file of selected.files) {
    copyFileSync(join(selected.dir, file), join(generatedPublicDir, file));
  }

  console.log(
    `[prepare-procedural-assets] copied ${selected.files.length} generated asset(s) from ${selected.dir} to ${generatedPublicDir}`
  );
}

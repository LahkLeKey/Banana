...
function readModelMetrics(metricsPath: string): ModelMetrics {
  ...
  try {
    ...
    if (parsed.schema_version !== TRAINING_ARTIFACT_SCHEMA_VERSION) {
      throw new Error(`unsupported_metrics_schema_version:${String(parsed.schema_version)}`);
    }
    ...
    return {
      min_signal_score:
        typeof metrics.min_signal_score === "number"
          ? clamp01(metrics.min_signal_score)
          : undefined,
      min_f1: typeof metrics.min_f1 === "number" ? clamp01(metrics.min_f1) : undefined,
      holdout_f1: typeof metrics.holdout_f1 === "number" ? clamp01(metrics.holdout_f1) : undefined,
      holdout_accuracy:
        typeof metrics.holdout_accuracy === "number"
...
function parseTrainedVocabulary(jsonText: string, artifactPath: string): TrainedModel {
  ...
  if (parsed.schema_version !== TRAINING_ARTIFACT_SCHEMA_VERSION) {
    throw new Error(`unsupported_vocabulary_schema_version:${String(parsed.schema_version)}`);
  }

  if (typeof parsed.generated_at_utc !== "string" || parsed.generated_at_utc.length === 0) {
  ...
  if (typeof parsed.vocab_size === "number" && parsed.vocab_size !== parsed.vocabulary.length) {
    throw new Error(
      `vocab_size_mismatch:declared=${parsed.vocab_size},actual=${parsed.vocabulary.length}`
    );
  }
  ...
  for (let index = 0; index < parsed.vocabulary.length; index += 1) {
    ...
    if (!entry || typeof entry.token !== "string" || entry.token.trim().length === 0) {
    ...
    if (weight > 0) banana.add(token);
    else if (weight < 0) notBanana.add(token);
  }
  ...
  const metrics = readModelMetrics(resolve(dirname(artifactPath), "metrics.json"));
...
export function loadTrainedModel(force = false): TrainedModel {
  ...
  try {
    ...
    cached = buildBuiltinFallback(
      `artifact_parse_error:${error instanceof Error ? error.message : "unknown"}`
    );
  }
  return cached;
}
...
export type ScoredText = {
  banana_score: number;
  matched_banana_tokens: string[];
  matched_not_banana_tokens: string[];
  matched_token_count: number;
  token_count: number;
};

export function scoreText(text: string, model = loadTrainedModel()): ScoredText {
  ...
  for (const token of tokens) {
    ...
    weightedHits += weight;
    if (weight > 0) matchedBanana.push(token);
    else if (weight < 0) matchedNotBanana.push(token);
  }

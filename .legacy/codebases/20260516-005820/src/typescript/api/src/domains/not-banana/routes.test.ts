...
describe("not-banana routes", () => {
  test("scores text with trained artifact and exposes model metadata", async () => {
    ...
    try {
      ...
      expect(scoreDefaultPayload.matched_not_banana_tokens).toContain("plastic");
      ...
      expect(modelPayload.artifact_path?.endsWith("vocabulary.json")).toBe(true);
  ...
  test("exposes fallback reason when artifact schema is invalid", async () => {
    ...
    try {
      ...
      expect(payload.fallback_reason).toContain("unsupported_vocabulary_schema_version");

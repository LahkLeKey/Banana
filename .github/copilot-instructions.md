...
- Break broad work into helper-sized slices and prefer the narrowest helper agent that owns the touched files: banana classifier, native core, native DAL, native wrapper, API pipeline, API interop, React UI, compose runtime, workflow, technical writer, value-risk prioritization, or test triage.
...
- For frontend UX validation, default to the VS Code integrated browser/web view path so runtime checks, terminal diagnostics, and rendered UI verification stay in one window.

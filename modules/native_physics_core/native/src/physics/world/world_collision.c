...
static PhysicsWorldCollisionResolveHook s_collision_resolve_hook = collider_resolve;

void physics_world_set_collision_resolve_hook(PhysicsWorldCollisionResolveHook hook)
{
    s_collision_resolve_hook = hook ? hook : collider_resolve;
}

void physics_world_reset_collision_resolve_hook(void)
{
    s_collision_resolve_hook = collider_resolve;
}
...
#pragma omp parallel for schedule(static)
    ...
    for (i = 0; i < cols->count; i++)
    {
        ...
        b = body_by_id[b_id];
        if (a && b)
            s_collision_resolve_hook(&cols->items[i], a, b);
    }
}

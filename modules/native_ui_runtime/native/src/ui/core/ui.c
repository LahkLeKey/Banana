...
static void *ui_default_malloc(size_t size) { return malloc(size); }
static void *ui_default_calloc(size_t count, size_t size) { return calloc(count, size); }
static void ui_default_free(void *ptr) { free(ptr); }

static UiMallocFn s_ui_malloc_hook = ui_default_malloc;
static UiCallocFn s_ui_calloc_hook = ui_default_calloc;
static UiFreeFn s_ui_free_hook = ui_default_free;

void ui_set_alloc_hooks(UiMallocFn malloc_fn, UiCallocFn calloc_fn, UiFreeFn free_fn) {
    s_ui_malloc_hook = malloc_fn ? malloc_fn : ui_default_malloc;
    s_ui_calloc_hook = calloc_fn ? calloc_fn : ui_default_calloc;
    s_ui_free_hook = free_fn ? free_fn : ui_default_free;
}

void ui_reset_alloc_hooks(void) {
    s_ui_malloc_hook = ui_default_malloc;
    s_ui_calloc_hook = ui_default_calloc;
    s_ui_free_hook = ui_default_free;
}
...
UIContext *ui_context_create(int framebuffer_width, int framebuffer_height) {
    ...
    UIContext *ctx = (UIContext *)s_ui_malloc_hook(sizeof(UIContext));
    if (!ctx) return NULL;
    ...
    ctx->framebuffer = (unsigned char *)s_ui_calloc_hook(fb_size, 1);
    if (!ctx->framebuffer) {
        s_ui_free_hook(ctx);
        return NULL;
    }
    ...
    ctx->elements = (UIElement *)s_ui_malloc_hook(MAX_UI_ELEMENTS * sizeof(UIElement));
    if (!ctx->elements) {
        s_ui_free_hook(ctx->framebuffer);
        s_ui_free_hook(ctx);
        return NULL;
    }
...
void ui_context_destroy(UIContext *ctx) {
    ...
    for (int i = 0; i < ctx->text_field_count; i++) {
        s_ui_free_hook(ctx->text_field_buffers[i]);
    }
    
    s_ui_free_hook(ctx->framebuffer);
    s_ui_free_hook(ctx->elements);
    s_ui_free_hook(ctx);
}
